...
(async () => {
  ...
  try {
    for (const width of [1440, 390, 320]) {
      ...
      await doc.getByRole('button', { name: 'Edit intent', exact: true }).click();
      const input = doc.getByRole('textbox', { name: 'Your project intent' });
      const original = project.summary;
      await input.fill('My preserved draft');
      project.summary = 'New context from another user';
      await doc.getByRole('button', { name: 'Save intent', exact: true }).click();
      await doc.getByText('Current saved intent', { exact: true }).waitFor();
      assert.equal(await input.inputValue(), 'My preserved draft');
      assert.equal(writes[0].expected_summary, original);
      assert.equal(await doc.getByRole('button', { name: 'Save intent', exact: true }).isDisabled(), true);
      ...
      await doc.getByText('Intent saved.', { exact: true }).waitFor();
      ...
      await page.getByRole('button', { name: 'Edit intent', exact: true }).waitFor();
      await page.getByRole('button', { name: 'Edit intent', exact: true }).click();
      await page.getByRole('textbox', { name: 'Your project intent' }).fill('Project index clarification');
      await page.getByRole('button', { name: 'Save intent', exact: true }).click();
      await page.getByText('Intent saved.', { exact: true }).waitFor();
