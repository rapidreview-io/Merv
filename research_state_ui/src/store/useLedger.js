...
function useLedger(fetcher, deps, pollMs) {
  const [data, error, refetch] = useRecordStatus(fetcher, deps);
  useIntervalPoll(refetch, pollMs, { enabled: pollMs > 0 });
  return [data, error, refetch];
}

export const useReflectionLedger = (projectId, pollMs = 0) =>
  useLedger(() => api.getReflections(projectId), [projectId], pollMs);
...
export function useArtifactLedger(projectId, pollMs = 0) {
  const [data, error] = useLedger(() => api.listArtifacts(projectId), [projectId], pollMs);
