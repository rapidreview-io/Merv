...
`date_from` is not after `date_to`, and unknown keys are refused. An arXiv ID
...
many are left out; `nisa.paper` names up to 100, `paper.cite`'s limit. `score`
...
web search, for literature, name `paper.cite`, and say that paper text is
...
| `nisa_not_found`                                                          | 404    | Nisa has no such paper                                   |
...
| `nisa_index_unavailable`                                                  | 503    | Nisa's index is down (HTTP 503 or 504)                   |
...
flight; the next waits its turn for up to `queueMs` (15 s) and is then refused
with `nisa_busy`. Retrieval has no per-call cost for a signed-in key (semantic
...
search counts as that account's. Anonymous accounts cannot mint keys. Mint it
once, signed in to that account, with `POST /api/sdk/auth/api-keys` (Nisa
returns the raw key a single time), and keep it only in the private env file.
...
its own model. Nisa's `/api/sdk/paper/{id}/pdf` is also left out: agents read
papers through Merv's own paper tools.
