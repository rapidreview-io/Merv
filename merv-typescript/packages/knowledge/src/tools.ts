  ...
  apply(ctx: Context) {
    ...
    ctx.effect(() =>
      ctx.tools.register({
        ...
        description:
          'Read all project claims, tasks and experiments, including terminal work, as metadata. Includes the current project Introduction and explicit publication availability. Does not read artifact bytes or advance workflows. Gate, blockers and next actions use workflow.status_and_next.',
    ...
    ctx.effect(() =>
      ctx.tools.register({
        ...
        description:
          'Resolve up to 200 project-scoped references to exact record metadata. Accepts record IDs or claim:, task:, experiment:, artifact:, review:, code-proposal:, code-commit: and session-final: followed by an ID. Each result explicitly identifies resolved, missing, unsupported or unpublished references. Published reflection references remain unavailable until a real reflection is published. Does not read artifact bytes.',
