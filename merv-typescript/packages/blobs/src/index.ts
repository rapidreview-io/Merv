...
export class DiskBlobs implements Blobs {
  ...
  put(namespace: string, bytes: Uint8Array) {
    ...
    catch(error) { if ((error as NodeJS.ErrnoException).code !== 'EEXIST') throw error; this.get(namespace,hash) }
    finally { unlinkSync(temporary) }
    return {hash,size:bytes.byteLength}
  }
  get(namespace: string, hash: string): Buffer {
    let bytes: Buffer
    try { bytes = readFileSync(this.path(namespace,hash)) }
    catch(error) { if ((error as NodeJS.ErrnoException).code === 'ENOENT') throw new MervError('blob_not_found','Blob not found',404); throw error }
    check(createHash('sha256').update(bytes).digest('hex') === hash,'blob_corrupt','Stored blob failed its integrity check',500)
    return bytes
  }
}
export const blobsPlugin = { name:'merv-blobs', apply(ctx: Context, config:{root:string}) { ctx.provide('blobs',new DiskBlobs(config.root)) } }
