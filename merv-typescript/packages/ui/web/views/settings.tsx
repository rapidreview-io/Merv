import { KV, ObjId, StatusPill, Table } from '../components';
import { useSession } from '../session';
import type { ViewProps } from './index';
import { ProjectIntroduction } from './project-context';

export function SettingsView({ shell }: ViewProps) {
  ...
  return (
    <div className="page-stage stack stack--lg">
      ...
      <details className="fold">
        <summary>
          Plugins · {shell.plugins.filter((p) => p.state === 'active').length} active
        </summary>
        <div className="stack">
          ...
          <h3 className="section-title">Sidebar rows</h3>
          <Table
            rows={shell.rows}
            keyOf={(r) => r.id}
            columns={[
