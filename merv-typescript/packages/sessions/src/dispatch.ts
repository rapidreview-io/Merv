...
export interface DispatchReading {
  /** An operator may pause, start and halt, and is told which ready work nobody took. */
  operator: boolean;
  ...
  /** Queued steps, counted only where the count says something. */
  waiting: number | null;
  stall: { code: DispatchStall; machine?: string } | null;
  /** Targets whose launches failed at their current revision: held, or still retried. */
...
export class SessionDispatch {
   ...
   * or failing target and a run of deferred closes are read at the record's current revision
   * where no session holds it, so every reader sees the same marks; nothing decodes a whole
   * lease. The candidate scan is the one costly read, so it runs only where its count says
   * something: when work could be waiting for a reason, and for an operator, who is told what
   * ready work nobody took. Only reads, on the caller's snapshot.
   */
  async running(caller: Caller, tx: Transaction): Promise<DispatchReading> {
    ...
    const admissible =
      !dispatch.enabled || idle || refusing || operator ? await this.candidates(caller, tx) : null;
    ...
    for (const row of closes) {
      const key = `${row.instance_id}:${row.revision}`;
      if (!row.closed_at || row.closed_at <= recent || failing.has(key)) continue;
