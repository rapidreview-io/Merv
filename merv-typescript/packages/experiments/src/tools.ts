    ...
    for (const definition of [
      {
        ...
        description:
          'Create a research experiment in the selected project with an immutable name, intent, optional details and tested claim IDs. Dependencies are work-item IDs in the same project. Starts planning attempt 1; at most seven experiments may remain active. Optional workspace "git" (requires Code) runs the experiment in a private Git checkout. Its base is derived from the code the dependsOn tasks were accepted with, looking through tasks that succeeded without code, or the project’s main when there is none; it is pinned when the first planning or running worker is leased, so execution starts from what the plan was written against. Tasks accepted with different commits are not merged yet: the experiment then shows code_merge_required, and code_base_pending while a base cannot be derived (an administrator binds the project with code.local.bind), and neither is launched. Optional baseTaskId is the older explicit form: it names one Git task, which must also be in dependsOn, whose accepted delivered commit becomes the base. Reuse the same requestId and input to recover a committed response.',
