...
export class EnvironmentCredentials implements CredentialProvider {
  ...
  resolve(caller: Caller, mountId: string): ResolvedCredential {
    ...
    const binding = this.#bindings.get(bindingKey(caller, mountId));
    check(
      binding,
    ...
    // Detect accidental reuse of an active local credential. An unknown token
    // still needs to have been issued by the upstream service through operator configuration.
    let local = false;
    try {
      this.#scope.authenticate(secret);
      local = true;
    } catch (error) {
      if (!(error instanceof MervError && error.code === 'unauthorized'))
        throw new MervError('credential_unavailable', 'Upstream credential validation failed', 503);
    }
    check(
      !local,
      'credential_unavailable',
      'An active Merv bearer credential cannot be used for an upstream service',
      503,
    );
...
export const credentialsPlugin = {
  ...
  inject: ['scope'],
  apply(ctx: Context, config: CredentialsConfig = {}) {
    check(
      plainObject(config) && Object.keys(config).every((key) => key === 'bindings'),
      'invalid_credential_config',
      'Credentials configuration only supports bindings',
    );
    const bindings = config.bindings === undefined ? [] : config.bindings;
    check(
      Array.isArray(bindings),
      'invalid_credential_config',
      'Credential bindings must be an array',
    );
    ctx.provide('credentials', new EnvironmentCredentials(ctx.scope, bindings));
  },
};
export default credentialsPlugin;
