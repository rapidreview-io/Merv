...
History keeps its 128 KB budget but never drops the newest exchange: one too long to send whole keeps each long text's start and end, and the checkpoint holds exactly what the next turn sends, so a long answer never outgrows it. A page opened mid-answer reads the whole answer so far from the snapshot, not only the stream's 64 KB tail.
