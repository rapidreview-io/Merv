...
- **Instructions.** Main sends them with each turn: an opening (the person's own agent, Merv's agent, never ChatGPT), `mainAgentGuide` (shared with MCP clients) and a closing on proposals, tool names and answers. Each turn's notes name its model (and that earlier answers may be other models'), the person (actor id, role) and project it serves, today, its machine, empty Problem sections and Introduction for people who write, and what a stopped answer had made: at most 6 such notes and 3 of the machine's, under the worker's 10. The relay, not the worker, sets each call's reasoning from the model catalog.
