...
- One active turn per conversation and one retained runtime per authenticated user. Stable command IDs, worker nonce, source/project/conversation/epoch/runtime/expiry checks, ambiguous-begin interruption, and cancellation fences.
- Separate Scope conversation authority and ToolRegistry conversation policy. Session authority remains separate. Native read handlers are admitted through the existing registry, including post-snapshot authorization. Missing authority fails closed; writes, mounted tools, shell/filesystem tools and nested delegation are excluded.
...
Schema limits, registry snapshots and final authority checks remain in force even when a tool advertises itself as read-only. These are the only allowed native tools. Conversational mutations remain separately scoped future work.
