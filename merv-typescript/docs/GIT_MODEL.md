...
- **Tools:** no tool is added for checks — `code.repository.configure` takes the specification and `code.base.retry` is the recheck. `code.repository.rebind` changes a hosted project's repository identity. Audit verification, storage migration and legacy retirement remain.
- **UI:** sandbox availability, audit completeness, backup/storage health, migration/rebind progress.
- **Tests:** the check state machine, its sealing and its reservation against a scripted adapter, and the route sequence against a recorded transport. Sandbox escape boundaries and termination are not testable against anything reachable without a cloud account and are not attempted here. Rebinding is covered by its verification (base pins and reviewed consolidation acceptances included), its busy refusals (a session another authority leased, a consolidation still holding a frozen set), its relaxed trigger in both dialects, its marker, a crash between the marker and the transaction, and the machine cache the driver re-keys rather than bricks. Storage migration/restore and old session drain remain.
- **Real GitHub:** separate Transport/Publisher identities, full auditing identity, ruleset bypass tests, rebinding verification.

Repository rebinding changes the opaque Code-side identity a hosted project is bound to. It is not a repository move and not a GitHub change: `code_projects.repository_id` carries no URL and no relation to `code_github.repository_json`, Code's own repository is keyed by the project alone and stays where it is with every object it holds, and the linked repository is not touched. What `code.repository.rebind` verifies before it writes anything is that Code's own repository holds every commit the project has retained as authoritative — main, every accepted commit (a unit's own and every reviewed consolidation round's), every unit head, every commit a unit's base is pinned to and every resolved base result — and a commit it does not hold names itself and refuses the whole rebind. An unhosted project cannot be rebound at all. Acceptances and base pins made under a previous repository stay valid because `binding_json` retains every repository the project has been bound to, consulted where an acceptance's repository is judged: `derive()` and consolidation's candidate freeze and re-validation. A frozen candidate set never spans a rebind, because one that is still outstanding refuses it. Moving objects between volumes, changing object format and separate publication identities remain unimplemented.
