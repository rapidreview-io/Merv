...
This release deploys the automatic-base, shared-repository and merge work together (S1–S3). None has been deployed before it. S4-1 adds executable workspace declarations. S4-2 implements frozen consolidation candidates, ancestry decisions and exact review binding, but not the retained-frontier workspace or consolidation acceptance. The rest of S4 and S5 remain unimplemented. Operator procedures are in [CODE_OPERATIONS.md](CODE_OPERATIONS.md), and checkout and capability rules in [WORKSPACES.md](WORKSPACES.md) and [MACHINE_RUNNER.md](MACHINE_RUNNER.md).
...
S4-2 registers unreleased `consolidation@5`, with recipes `consolidation.consolidating@3` and `consolidation.consolidation_review@3`. Versions 1–4 retain their published definitions, execution policies and recipes. Version 5 currently has `consolidating → consolidation_review → complete`, a revision edge back to `consolidating`, and abandon/failure edges from both live states. It completes at approval with publication outstanding. S4-3 will change this same unreleased version in place to add the publication and stale-base states above.

This partial slice implements candidates, decisions and review binding. Explicit `version: 5` requires a bound project with imported main held by Code. Default Git creation, including Research's unchanged handoff, remains version 4. Version 5 still uses the existing producer workspace and records no unit acceptance; it is not ready for automatic rollout. The remaining workspace slice must introduce the decision-before-production transition, derive the producer's base from the retained frontier through the existing planner, and record its acceptance at approval. Research will continue passing generic scope and workspace declarations; Consolidation will own version selection by asking Code when that path is ready.

The version-5 scope is the declared dependency closure of selected experiments, explicit cycle `taskIds`, and consolidation prerequisites, excluding system edges. All accepted tasks and experiments in that closure are frozen, including accepted ancestors below code-bearing candidates; unaccepted units are omitted. The explicit task list includes cycle work that no selected experiment depends on. This avoids silently losing such work or unrelatedly sweeping every task in the project. Research's version-5 handoff, including that task list, remains part of the workspace slice. The existing immutable consolidation record carries the versioned candidate set, its hash and frozen integration base; no table or migration is added.
...
The existing immutable submission and sealed-proposal payloads carry the versioned decision manifest and its hash. The review pins the candidate hash, manifest hash, integration base, exact receipt head/tree and evidence through its sealed manifest artifact and provenance certificate. Its required criteria cannot be waived on a pass. Code resolves contributors from accepted commits in the retained frontier's ancestry, and shares the Sessions contributor reader with resolution reviews. A new `code.consolidation` certificate opts into transactional revalidation at both claim and verdict; existing certificates retain their original rules. Ordinary reads still work without Code, while a version-5 claim or verdict refuses without its certificate owner. The submission service computes ancestry, frontier, conflicts and the contributor proof before opening its committing transaction. Acceptances, the frozen candidate set and commits are immutable, so this proof remains valid while submission checks the candidate-set hash, manifest hash, decisions hash and reconciliation coverage. Claim and verdict reuse the pinned ancestry projection and recheck contributor identities through the existing provenance mechanism; neither runs Git. The returned graph excludes every commit reachable from the frozen integration base; branches from older main stop at that main history too. Targeted identity queries retain ancestry decisions and accepted contributors predating main without returning its history, while unrelated candidate histories are refused. Inspection keeps a shared 10-second Git deadline, 32 MiB output budget and one million graph visits. No merge or publication operation was added.
