...
**Stage S1 (automatic single base, local mode) is implemented,** and S2 with it (below); S3 is complete. S4 and S5 are not. The operator's view is in [CODE_OPERATIONS.md](CODE_OPERATIONS.md#units-acceptance-and-automatic-bases) and the checkout rules in [WORKSPACES.md](WORKSPACES.md#the-derived-base).
...
**Stage S2 (shared Code remote) is implemented.** S3 is complete; S4 and S5 are not. The operator's view is in [CODE_OPERATIONS.md](CODE_OPERATIONS.md#the-code-repository), the machine's in [WORKSPACES.md](WORKSPACES.md#the-codev2-driver) and [MACHINE_RUNNER.md](MACHINE_RUNNER.md#workspace-drivers-and-capabilities).
...
**Stage S3 is complete: automatic bases, bounded server merges, and one recoverable resolution task per conflict.** `code.repositories.autoMerge` defaults to true; setting it to false keeps work needing a merge visibly blocked.

- **Base records and planning.** `code_bases` retains one record per distinct set of accepted commits in a project. Its key is the sha256 of sorted full commit ids; lookup also compares the retained member list. Triggers freeze the two planned inputs, engine and sponsorship, make the result write-once, and prevent deletion. The pure planner (`packages/code/src/base-plan.ts`) chooses the largest contained record, then whichever adds the most missing members, breaking ties by key. Each merge joins exactly two records; unresolved records are built on, quarantined records are excluded.
- **Server merge.** Code runs `merge-tree --write-tree` and `commit-tree` outside database transactions, with fixed settings shared with its driver and a fixed server identity/time. Conflicts retain paths and Git diagnostics; containment reuses the descendant. Work runs once per project at a time, with an increasing execution epoch and a two-minute deadline covering the whole merge. It recovers after crashes and stops after five admitted infrastructure attempts in `blocked_infra`. An overrun is abandoned and retried under a new epoch; a late result cannot seal that epoch. A create-only `refs/merv/bases/<key>` precedes conditional result sealing.
- **Derivation and waiters.** Code-hosted units derive a `merged` base from resolved records. Reconciliation records unresolved plans and attaches the same service-owned prerequisite to every direct, indirect and future waiter. System dependencies never contribute code to derivation and never make waiters fail. Blockers and stuck reports name the work, its current state, and the operator's recovery options. Runner-owned repositories retain `code_merge_required`.
- **Resolution task and rounds.** A conflict creates one unpublished `task@7` on one branch, linked transactionally to its base. Its per-project service actor has producer permission, no credential and no review eligibility; project writers may lease the producer assignment without gaining operator authority. The bounded brief identifies both frozen inputs, contributing work, conflicts, procedure and required evidence. Returned reviews retain submissions and feedback through Reviews, task checkpoints and workflow history. The existing `rejectedReviewIds`/`revisionContext` carry-forward includes earlier review notes, verdicts, unmet criteria and submission references; context omissions report feedback dropped by the budget. Code writer generations retain lease start heads. No Tasks round table or copied Code checkpoint exists.
- **Suspension and recovery.** A `fail` verdict or `task.mark_failed` suspends service work, including abandonment. The existing `review_rounds` budget defaults to three and counts producer exits (delivery or withdrawal); exhaustion suspends the same task. Only a signed-in human administrator may resume it by extending the limit, with grant and transition committed together. Alternatively an operator can cancel/replan waiters. Open conflicts and suspended work are not timer jobs.
- **Portable merge and driver.** `code_pending_merges` retains the frozen plan, first-parent lineage and second parent, current checkpoint and write-once first merge. The strict `code.v2` manifest/snapshot carries this state; downloads include both inputs. Only the service producer policy grants `code.merge`. Its `start` requires the clean initial checkpoint and journals the merge tree before materialising conflicts. `complete` creates exactly two ordered parents even for an unchanged tree. Ordinary commit/capture saves single-parent WIP, allowing another machine to resume without `MERGE_HEAD`; later rounds make ordinary corrective commits. Metadata lives on the existing local workspace row and the start receipt on its transfer row.
- **Admission and acceptance.** Upload admission verifies the first-parent walk and frozen second parent outside transactions, then retains immutable proof in its receipt. Acceptance checks that proof against the frozen plan inside its transaction; the base worker independently verifies Git before sealing `method: task`. An incorrectly parented first merge cannot be repaired by a later merge. Ref creation and sealing recover idempotently; refused acceptances remain visibly unresolved. Service submissions retain authenticated runner authority. Code also requires the passing review’s pinned provenance to match the current retained contributors before accepting the unit.

- **Independent review.** Code walks the frozen plan and looks up each member commit’s hash-checked acceptances and each intermediate resolution task. Sessions supplies their retained producing actors and directing authorities, including earlier rounds; Code reads only its own tables. No new table is needed. A registered owner capability supplies a canonical certificate once when Reviews creates the request; `reviews@9` adds its immutable nullable versioned field on both backends. Reviews checks the reviewer and current directing authority against the pinned certificate at claim and submission through the existing exclusion rule. Inputs are terminal and resolution work cannot acquire a lease while in review, so no producer can join those units during an open review. Code recomputes once at acceptance; a later unit accepted with the same member commit expands its provenance and causes acceptance to be refused. An operator reading a requested review or its task sees when all eligible reviewers are excluded. Pinned and ordinary reviews remain usable with Code unloaded.

The additive `code_pending_merges@1`, `session_service_work@1`, `code_bases@4` and `code_units@5` migrations have SQLite and PostgreSQL definitions. Unreleased task@7 fingerprints deliberately reflect its suspension graph and service merge grant; published versions remain unchanged. Both manual and automatic offers require the declared driver.

- **Admission and usage.** Sessions owns `session_service_work`, keyed by provider, operation id and execution epoch. Its server-only capability reserves project capacity before execution, checks dispatch and the existing project/root budgets, and settles wall usage exactly once in the same transaction as Code's outcome. `sessions.serviceConcurrency` defaults to one per project across providers. No worker session or model tokens are invented. A lost process holds its reservation until the deadline and is conservatively charged that duration; Sessions sweeps expired work even with Code unloaded. Missing worker usage still withholds cost/token budgets through the existing accounting rules.
- **Sponsorship.** Code freezes the roots of all current waiters whose requested sets contain the planned step when it creates the record. Workflows supplies roots through its dependency and policy-child graph; Code reads only its own tables. Subsequent waiters reuse the record without changing its sponsors. The project pays once and each frozen root sees the entire shared wall cost despite later dependency changes. `code_base_admission` carries dispatch, capacity, budget, usage or missing-capability waits through workflow blockers and `session.stuck`. These waits consume neither infrastructure attempts nor launch/review limits.
- **Base operator controls.** `code.base.retry`, `code.base.suspend`, `code.base.resume`, `code.base.cancel` and `code.base.quarantine` share one transactional transition path and request-fingerprinted receipts in `code_operations`. A human or operator key needs project admin permission; no leased worker receives that power. Retry restores five infrastructure attempts; resume restores prior work without resetting review rounds. Cancel and quarantine retain identity/history. Quarantine follows base inputs, pins and acceptances, fences affected writers and blocks their future mirror publication. Unrelated work continues. A sealed result is never replaced or made healthy by retry.
- **Mirrored bases.** Resolved automatic and reviewed bases join the existing asynchronous journal as `mirror-base`, publishing create-only `refs/heads/merv/bases/<key>` on GitHub. Startup enqueues already resolved records too. Server-held credentials, divergence checks and retry controls are shared with the existing mirror. Completion never gates a base, lease or acceptance.

**Deploying S3.** Deploy Sessions and Code together after S2's repository import, then deploy machines carrying the pending-merge `code.v2` driver before allowing resolution work. Automatic merging defaults on; an operator turns project session dispatch on when ready, or keeps `code.repositories.autoMerge: false` during rollout. Set project/root budgets and `sessions.serviceConcurrency`. Watch `code.status.bases`, `session.stuck` admission reasons, infrastructure attempts, suspended resolution tasks and mirror lag. Back up the database with the Code repositories. Pre-S3-b4 unfinished records acquire frozen sponsorship when first admitted, without inventing historical usage. Executable checks and their sandbox remain S5.
...
- **Tools:** `code.status.bases`, `code.base.retry`, `code.base.suspend/resume`, `code.base.cancel`, `code.base.quarantine`, and `workflow.extend_limit` for service-task review limits. Service admission is a server capability, never a worker tool.
...
- **Gate:** complete; automatic merging defaults on and respects project dispatch, capacity and budgets.
