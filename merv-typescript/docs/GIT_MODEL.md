...
**Stage S3 is partly implemented: frozen base plans, server merges, and one recoverable resolution task per conflict.** Automatic merging remains switched off (`code.repositories.autoMerge`, default false) until provenance-aware review and session-less admission/budgets are implemented.
...
Remaining: provenance-aware review, session-less admission/budgets, mirrored base refs, and base-record operator tools (`retry`, `suspend`, `cancel`, `quarantine`).
