...
**Stage S2 (shared Code remote) is in progress.** Shipped so far: the repository Code keeps for each project on the server's disk, closure admission, the operation journal with crash recovery, and import (`code.repository.import` from an operator's bundle or the linked GitHub repository, `code.repository.configure`, the `code-import` command, the `/code/v2/uploads/*` routes). Also shipped: writer generations with the fence on every upload, the source-authenticated final capture, `code.unit.fence`, downloads of exactly the canonical head or exactly the referenced commit, `task@6`/`experiment@9` with the opaque `driver` key of a workspace policy, runner capabilities (`code.v2`) with `runner_incompatible`, and the Code-owned workspace driver the runner is composed with. The operator's view is in [CODE_OPERATIONS.md](CODE_OPERATIONS.md#the-code-repository) and [CODE_OPERATIONS.md](CODE_OPERATIONS.md#writer-generations-and-fencing), the machine's in [WORKSPACES.md](WORKSPACES.md#the-codev2-driver). Not yet: `preparation_deferred` as a session outcome (the driver already classifies a deferral; the runner still releases it as `workspace_failed`), mirroring to GitHub, and the mirror's visibility. Once a project is imported, new Git work that names no `baseTaskId` is created on `task@6`/`experiment@9`; work already under way finishes on the version and in the repository it began with.
