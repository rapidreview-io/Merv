...
**Stage S1 (automatic single base, local mode) is implemented,** and S2 with it (below); S3 is partly implemented. S4 and S5 are not. The operator's view is in [CODE_OPERATIONS.md](CODE_OPERATIONS.md#units-acceptance-and-automatic-bases) and the checkout rules in [WORKSPACES.md](WORKSPACES.md#the-derived-base).
...
**Stage S2 (shared Code remote) is implemented.** S3 is partly implemented; S4 and S5 are not. The operator's view is in [CODE_OPERATIONS.md](CODE_OPERATIONS.md#the-code-repository), the machine's in [WORKSPACES.md](WORKSPACES.md#the-codev2-driver) and [MACHINE_RUNNER.md](MACHINE_RUNNER.md#workspace-drivers-and-capabilities).
...
**Stage S3 is partly implemented: frozen base plans, server merges, and one recoverable resolution task per conflict.** Automatic merging remains switched off (`code.repositories.autoMerge`, default false) until session-less admission/budgets are implemented.

- **Base records and planning.** `code_bases` retains one record per distinct set of accepted commits in a project. Its key is the sha256 of sorted full commit ids; lookup also compares the retained member list. Triggers freeze the two planned inputs and engine, make the result write-once, and prevent deletion. The pure planner (`packages/code/src/base-plan.ts`) chooses the largest contained record, then whichever adds the most missing members, breaking ties by key. Each merge joins exactly two records; unresolved records are built on, quarantined records are excluded.
- **Server merge.** Code runs `merge-tree --write-tree` and `commit-tree` outside database transactions, with fixed settings shared with its driver and a fixed server identity/time. Conflicts retain paths and Git diagnostics; containment reuses the descendant. Work runs once per project at a time, recovers after crashes, and retries infrastructure failures five times before `blocked_infra`. A create-only `refs/merv/bases/<key>` precedes conditional result sealing.
