...
The 2026-09-22 inventory has 55 plugin entrypoints: 25 service providers, 14 tool adapters, 14 UI adapters and two API adapters. It records 145 direct Cordis dependencies, including 13 optional integrations. There are 54 server entrypoints and one independent machine entrypoint. The default server configuration selects 49 entries; API-only selection uses 36. Optional bindings include Experiments, Knowledge and Tasks → Code Research, and Research → its stage providers. The retired Consolidation and Claims plugins are absent.
