...
Nisa is implemented and its Fable review is complete; its real-service gate remains open. The default configuration has eighteen entries; Mounts and Nisa are explicit additions. The image represents implemented declarations, not a claim that every optional plugin is currently running in one persistent server.
