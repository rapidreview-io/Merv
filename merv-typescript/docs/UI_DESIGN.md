...
The founder found the labels were not a vocabulary — `New claim`, `Start a reflection wave`, `Start consolidation`, `Write one`, `Claim this review`, `Enable dispatch`, `Halt every live lease`, `Advance when ready`, `Prepare download` — so a label never predicted what it powered. From this wave every control that acts reads `<Verb> <object>` in sentence case, from this table and nothing else. Never a sentence, and never a second phrasing of the same act: the fold opener, the form's heading and the form's own button all read the same words, so what a person clicks is what the form says it will do. Secondary phrasing inside a sentence stays.

| Verb     | What it does                                                                                             | Objects                                                                                                        |
| -------- | -------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| New      | creates a record                                                                                         | New claim · New cycle · New reflection · New consolidation · New section · New citation · New member · New key |
...
| Halt     | stops agents or leases                                                                                   | Halt lease · Halt all leases · Halt dispatch                                                                   |
...
- **Act is a ladder, not a paragraph.** Every record with a workflow instance shows its program's own states in order, each marked _done_, _here_ or _next_, with the time it was entered; the one control that acts stands beside it. The rungs and the marks come from `workflow.process`, which the row's own `ui.read` serves with the record (`packages/experiments/src/ui.ts`, `packages/tasks/src/ui.ts`), so a record page never asks a second time for its gate. No blocker is printed: each is an instruction to the agent holding the tool. Unsettled prerequisites keep their names and state words.
