...
"Every time you are showing a workflow in the UI, please show it as a diagram with a visual of where we are at on that diagram." One component draws it (`packages/ui/web/process.tsx`): the program's states left to right in the order its own edges reach them, ends last and capped, arrows between them and a dashed curve beneath for every way back, the record's place filled in its kind's colour and quietly pulsing, what it has passed filled muted, what it has not reached an outline. The record page draws its own process graph; a list row draws the same machine compact — dots on a line, the state word on hover — from the deployed definitions `ui.shell` now carries, so no row costs a request.
