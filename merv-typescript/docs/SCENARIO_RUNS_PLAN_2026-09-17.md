...
14. Three concurrent runs saturate the schema-wide advisory lock on the 2-vCPU host: a round 4 planner's `artifact.create` timed out three times and exited without submitting, and runners flapped offline with `control_unavailable`. Harness observation and runner polling slowed to 8 s, lock timeout 45 s, statement timeout 60 s (release 2026-09-17 ~19:00Z). The lock itself (one per schema, for commit-ordered event ids) is the ceiling; a per-project lock is the real fix and stays on the list.

15. The founder found every page load "slow AF". Core: the Postgres state layer takes one advisory lock per schema in every transaction, reads included, and every service method behind a page is a transaction, so page reads queue behind agent writes (one row read measured 5 s under three runs). Fix (release 2026-09-17 ~19:45Z): every read-only tool runs in a snapshot scope, `BEGIN READ ONLY` with no lock, whose nested component transactions read on one snapshot and are refused if they write; `read()` itself is unchanged because services use it to join an ambient transaction; SQLite keeps plain execution since its single serialised connection would deadlock. Two hot sequential scans (actors 3.3M, events 288k) are cheap on their tiny tables and were left alone.

16. The boot's GET routes (`/account`, `/sessions/status`, `/code/github`, `/projects`) bypass the tool registry and were still on the writer lock: 2 to 7 s per call under three runs. Every read-only GET now runs in the snapshot scope (release 20260917T195812Z); the same calls measure 0.24 s, `/sessions/status` about 1 s. Remaining page cost is the browser's own fan-out (Home fires about 20 requests after a five-request boot) and the host's CPU; both are in flight.

17. Release 20260917T202510Z carries the UI waves from the walk-through: Home in one read (about 35 requests → 2), the boot in three requests, and the seven fixes (no heading over the ladder, Now without the Introduction, no error codes or raw tokens on Home, compact claim rows, no identifier in the rail footer, a plain chooser). Verified in the founder's signed-in pane. Next UI wave in flight: every workflow drawn as a diagram with the current position marked.

18. A gpt-6-astra Codex task designed a finer writer lock (project writers take the schema gate shared plus an exclusive per-project lock; global writers take the gate exclusively; event appends take a separate events lock from the first append so ids stay commit-ordered) across 16 files. The full suite fails 10 tests with it (feed drain, mount removal, legacy version negotiation, catalog withdrawal, workflow withdrawal), so it is parked as `dev_docs/patches/codex-per-project-lock-2026-09-17.patch` (1,168 lines) and stays out of HEAD until those pass and the Postgres-backed state tests run against a database (`MERV_TEST_POSTGRES_URL`; no local Docker daemon tonight).
19. `ui.home` read its fifteen parts sequentially inside one pinned connection: 5 s under three runs. Its parts now read in parallel, each in its own snapshot scope (release 2026-09-17 ~20:55Z).

20. All three runs stalled for an hour from about 20:30Z: every runner-dispatched review launch died on Codex's 30 s MCP handshake and the runners flapped offline with `control_unavailable`. The database showed all ten pooled connections taken, six of them writers queued on the schema lock, so every read scope waited for a connection. Fix: a pool of 40 with a 30 s connection wait, and the runner's Codex profile waits 120 s for the handshake as the harness launches already did (release 2026-09-17 ~21:35Z; harnesses restarted after it). The schema lock remains the ceiling; the parked per-project lock is the real fix.

21. Every experiment plan in all three scenarios cycles in design review for one structural reason (scenario 1 `lora-vs-prompt` seven straight rejections, `data-scaling` four; scenario 2 `tokenizer-ab-150m` four; scenario 3 `awq-70b` two): the reviewer asks the plan to pin the prerequisite task's accepted, frozen deliverable, and `experiment.attach` refuses it with `invalid_evidence_author` because evidence must be authored by the attaching worker (`packages/experiments/src/index.ts`, the check in `attach` and again at submission). `pinnedInputIds` exists to carry exactly such inputs and is always empty in practice. The reviewers are right and the planner cannot comply, so no run can reach `running`. This meets the stop rule (three consecutive rejections for one reason). Scenario 1 ended at its four-round cap with a report at 22:20Z, 294 Codex executions. Proposed fix, not applied without the founder's ruling because it changes evidence provenance: under role `plan`, accept an artifact the worker did not author when it is an accepted delivery or accepted result of a settled record in the same project, pin it as an input, and put it in the planning session's readable set. Two smaller findings: scenario 2's context still says 400M observations while the accepted harness delivered 2B, which its reviewers flag as a contract conflict; scenario 3's `slice-evals` long-context slice holds 1 of 300 required items. All three runs were closed under the stop rule at about 23:00Z with their leases halted and a `report.json` each: 294, 247 and 107 Codex executions; scenario 3's `dense-32b` and `specdec-70b` were never planned.

22. The founder ruled on item 21 the same evening: "reviewers shouldn't have read limitations. Drop that." A plan references the approved artifact and the reviewer goes and reads it. The engine now admits `artifact.get`, `artifact.read` and `review.get` for any record of the project when the execution is read-only, which every reviewer session is, in every program and every published version; nothing else opens, a named id is still required, and no published policy changed (`admitRead` in `packages/workflows/src/index.ts`). Who is still limited: every worker session (task work, experiment planning and running, reflection lenses and synthesis, consolidation) reads only the artifacts its assignment names plus what it wrote, reads only its own review, and sees only its own record and declared dependencies; no sealed session, reviewer included, can list or browse records. People and interactive agents holding a key have no such limits.

23. Then the wider ruling: "Everyone should be able to read everything else. No read constraints for now." Every session, worker or reviewer, in every program and version, may now call every native tool that only reads, with any arguments: the registry marks such a tool as a read, the mark travels with the invocation, and the engine admits a read the policy never named, or named differently, as given (`admitRead`). Sessions list those tools beside their own. What still holds: every write grant and binding as published, a session's own submissions (an experiment submits the evidence it was offered plus what it authored, a figure it embeds must be a frozen input or its own), the project boundary, and evidence provenance. What the ruling gives up for now: a reflection lens can read a peer's report before synthesis, so lens independence is asked of the agent rather than enforced.

24. 2026-09-18 00:32Z: the founder said "Go for d8". The production VM `rp-control-1` was resized in place from Standard_D2s_v3 to Standard_D8s_v3 (8 vCPU, 32 GB) in five minutes; the public IP is static and unchanged; Merv, the database and the sandboxes stack restarted on their own. Runs were stopped and their leases halted before the resize and relaunched after it. Also on 2026-09-18: GET routes under /code/ left the read snapshot (the GitHub status route swept expired OAuth flows and failed with read_only_scope on production; release 20260918T000237Z); three private repositories `rapidreview-io/merv-ledgerline`, `merv-fathom-signals`, `merv-parlay` were created; one Git-workspace experiment was added to each brief (`scorer-in-repo`, `harness-in-repo`, `slices-in-repo`) and the harness runner gained a GitHub workspace (`--github`); the projects await the founder's OAuth connection and repository link. And scenario 1's `lora-vs-prompt` passed design review on round 9, the first plan approved, once reads were open, the introduction said so, and the model was named.

25. 2026-09-18 ~02:30Z, founder ruling on the 150M ordering: "we don't do experiment waits for another experiment, we only do task to experiment to task kind of relationships." Scenario 2 restructured accordingly: a task `rates-from-60m` depends on `tokenizer-ab-60m` and delivers the per-arm best learning rates; `tokenizer-ab-150m-2` and `posenc-150m-2` depend on that task; the original `tokenizer-ab-150m` (nine design rounds) and `posenc-150m` were abandoned with the ruling as the reason. Earlier the same night: the accepted `proxy-harness` entry point could not vary learning rate or seed, so `proxy-harness-v2` was added and delivered and passed review in one round; scenario 3's long-context slice was redefined as the 300 longest threads after the worker's scan found only 21 threads over 8,000 tokens; the harness now relaunches a network stage whose lease ended without advancing.

26. 2026-09-18 05:50–06:15Z, the UI "slow loading everything" (founder). Measured on production with a project key: ui.shell 2.6–4.6 s, ui.home 7.6–8.1 s (845 KB), a 1 KB paper read 5–6 s. Cause, from pg_stat_activity and pg_stat_statements: the schema-wide advisory writer lock was at full utilization (400 acquisitions/min, 0.8 s mean wait, ten waiters), and most holders were session admission checks and worker authorization checks that only read but ran in locked transactions; every worker tool call opened three to six of them. The holders idled between cheap queries while the Node event loop was busy, much of it serialising the home answer for a ten-second poll. Fixes, three releases: (a) session prepare/validate/tool listing and the worker authority check read on a snapshot (no lock); ui.home read-only again so its parts share one lock-free connection; an events (project_id, subject_id, type, id) index for the authored-in-session lookup; (b) ui.home trimmed to the fields the map draws (845 KB → 84 KB, no session history, no findings, no evidence). After: shell 0.5–0.9 s, home 0.5–0.9 s, paper 0.3–0.4 s, records 0.4–0.9 s, of which ~0.2 s is the round trip to the VM; lock mean wait 117 ms with one waiter. What remains: the lock is still taken by every observation write and every real write; the structural answer is a lazy writer lock (acquired at the first write, not at BEGIN) or the parked per-project lock, both needing a careful audit of read-then-write invariants.

27. 2026-09-18 ~07:30Z, "why so many rejections" (founder). 58 verdicts on record: 10 passes, 48 rejections, no fails. Causes by count: the plan could not pin evidence the reviewer could read (20, sealed sessions, since opened); the accepted apparatus could not do what the plan needed (13, proxy-harness v1 had no learning-rate, seed or arm flags, so a task passed its own review while failing every experiment on it); a plan written before its inputs existed (4, planning was not gated on dependencies, since fixed); cost above the operator's cap (3); the corpus could not meet the brief's number (2); genuine scientific catches (2); planted defects, wording and a broken upload (4). Reading: the reviewer was right almost every time and repeated one demand until it was met; in 33 of 48 cases the fix was outside the planner's reach and the loop had no exit that reached a person; each planning session inherited its own earlier verdicts, so once a plan wrote "this worker cannot read the harness evidence" the later rounds believed it even after reads were open; task acceptance never checked downstream fitness; operator decisions travelled only through the introduction and reached only the next lease. Product changes proposed, awaiting the founder's pick: a record rejected twice for the same demand stops resubmitting and surfaces a blocker on Now with the demand quoted; a planner may end a session with "needs an operator decision" instead of a submission; a task review includes its dependents' stated needs as criteria; operator notes reach a running session.

28. 2026-09-18 08:00–08:20Z. Headless Claude workers end their session and wait for a wake-up that never comes: the sweep worker on `tokenizer-ab-60m` replied "the background timer will wake me at about 08:38 UTC" and its lease ended with six GPU runs still training. Nothing wakes a `claude --print` session. Every Claude launch (harness and runner) now carries one sentence: the session ends the moment it gives a final reply, nothing wakes it later, so remote work is waited on inside the session before the handoff. The harness relaunches a network stage whose lease ended without advancing only after five minutes and at most eight times per record (commit 1745db54). Live leases were halted and all three harnesses restarted at 08:12Z so the next worker reads the sentence; the GPU jobs on the Thunder boxes ran on. Spend: scenario 2 $37 of $200, scenario 1 $6 of $40, scenario 3 $4 of $60.

29. 2026-09-18 08:55–09:50Z, "improve all the bugs of Merv by testing it inside and out, use claude cli to run the agents" (founder). Three sources of bugs, all fed by Claude CLI agents: (a) the scenario runs themselves, now recorded as stream-json so every tool call and error a worker sees is on file (`dev_docs/scenario-runs/0N/launches/*.jsonl`), plus a SIGUSR2 drain so a harness restart never loses a worker; (b) a local probe bed (the demo server on 3095/3096, SQLite, disposable) hammered by explorer agents with one lens each, API contract, workflows end to end, permissions and isolation, each with a checklist and a findings file; (c) a headless crawl of every UI route on production and on the bed collecting console errors, failed requests and error text. Round one found 42 things, 27 of them real, fixed and released in five releases (092002Z, 092627Z, and three more before 095000Z): input soundness before any tool runs (lone surrogates were echoed as written but stored replaced, NUL truncated SQLite text, a body nested 2,000 deep overflowed canonical() into a 500); bounded task title/goal/checks and review notes (a 200,000-character title made every dispatch label check fail so /sessions/status and automatic leasing answered 500 for the whole project); submit_design waits for prerequisites like begin does, and the guidance for an experiment whose prerequisite failed now says so; the initial state keeps its first entry after a revise edge; experiment results that outgrow the review recipe are listed instead of refused (this had stranded scenario 1's data-scaling in experiment_review with the runner degraded on context_too_large; it passed review and completed within minutes of the release); a five-minute operator token could rotate a non-expiring credential of its own into a fresh permanent one; a worker asking workflow.assignment for another record silently got its own back; feed.activity handed readers other principals' credential ids; another project's session or agent answered 403 instead of not found; an offer reported live-session and secret conflicts before checking the caller's authority; consolidation.create accepted any string as an experiment id; the owner's interactive delivery moved a task out from under its live worker; artifact.list returned the oldest 1,000 and hid newer ones; ui.home drew a post count from the oldest fifty posts; project.references reported research cycles as missing; a mixed-case media type read back as base64; a claim requestId with whitespace made a second claim; the archive refused project keys with a 403 the rail linked to; every page load fetched a favicon and logged a 401. Judged not bugs: a source delegating review of its own work to a fresh worker (by design, tested), the SDK's -32603 with a Zod dump on a malformed tools/call (the SDK validates before the handler; left alone), control characters other than NUL in names (logs with ANSI escapes are legitimate artifacts). Round two lenses (sessions and runners, research/reflections/paper/claims/knowledge/feed/code, concurrency and idempotency) launched at 09:52Z.

30. 2026-09-18 09:52–10:45Z, probe round two (lenses: sessions and runners; research cycles, reflections, paper, claims, knowledge, feed, code; concurrency and idempotency): 24 findings, 20 fixed in three releases (a4aef210, 7ed02743, 40f2392e). The ones that mattered most: every successful worker handoff was recorded as an expired session with reason revision_conflict, because the sweep saw the record move and never asked who moved it (now: the record moved by the worker's own hand closes the session as a completed handoff); one task whose brief outgrew its recipe wedged every automatic lease of its project, the first candidate always failing and nothing behind it tried (dispatch now moves past a candidate whose offer cannot be built and surfaces the failure only when nothing else is leasable); a worker's tool input over 256,000 characters was refused as 'execution metadata too large' while artifact.create allows two megabytes; readers could register runners against the project's limit and mint agent actors; a lens Summary followed directly by the next heading passed the nonempty check; project.references and feed.activity handed readers other principals' credential ids; a 200-character cycle name produced a 215-character consolidation name and a Zod dump; workflow.status_and_next called research.advance ready while the call refused with reflection_open; reflection.get kept its paper proposal unaccepted after the paper accepted it; actor.revoke on an inactive actor appended another event; two persistent agents on one runner collided on each other's request ids; a task worker could not review.get any review because its policy bound reviewId to a reference still null. Judged by design: a source (an operator's key) delegating review of its delegate's work, since independence is a different agent and context and the existing test says so. Left open, needs a workflow version: a research cycle whose selected experiment was abandoned has no edge to close or replan (research-2). Also this round: the harness runs a network worker beside its loop, so the runner keeps dispatching the project's reviews while a long stage runs (a results review had waited unclaimed for an hour behind the 60M sweep), and SIGUSR2 drains a harness. Scenario 1 finished all four records (eval-harness done; lora-vs-prompt, data-scaling, scorer-in-repo complete). Scenario 3: awq-70b and dense-32b complete, slices-in-repo in results review, specdec-70b running. Scenario 2: the 60M sweep worker has held its session since 08:16Z waiting for six Thunder runs; spend $76 of $200.

31. 2026-09-18 10:40–11:20Z, probe round three (lenses: the UI's data layer; the paper; a fresh worker's path, source unread) plus production crawls: 26 findings, 21 fixed in four releases (fbf8042c, 365edcb0, 5946de92, d2037661). The serious ones: a paper that grew within its own limits (two documents near 160,000 characters) made every experiment assignment and offer in the project fail with context_too_large, because the whole paper sat in the required experiment section (the paper is now embedded while it fits 40,000 characters and a longer section names its size for paper.read); the Feed page polled feed.activity without a cursor and got the oldest 1,000 events, so on production it had been blind to everything after the first thousand (without a cursor the newest page answers); every row link in a list shown beside an open record resolved under that record's route, a blank page (seen on production as /ui/artifacts/<a>/<b>); a research cycle whose selected experiment was abandoned had no way on, so `research.replan` reselects a cycle's work and the engine drops dependency edges in the same owner change (scenario 2's cycle was replanned onto the restructured work); task.create accepted goals and checks whose work context could never be built; NUL and lone surrogates reached published paper sections through change artifacts; a plan attached at a second path left the attempt with two current plans and no way to submit; a running worker discovered the report's four required sections and the exhibit filename through four failed submits (recipe v4 says them), and a task reviewer learned about the synopsis and findings the same way (task.review v2); abandon and mark_failed showed as ready and then demanded a reason; a leased worker was offered workflow.begin, which its policy does not hold; a producer whose task a worker held was steered to begin and then refused; POST /sessions/* named no invalid field; ten messages described a different problem than the one refused. Judged by design: experiment submissions are snapshots (a database trigger), so the paper is the authority on a proposal's acceptance; requiredInput does not list requestId (the tests encode the contract; the tool descriptions say it). Left open: task.get carries goal, checks and confirmations twice (top level and workflow.data), about 3× the necessary weight at the limits. Also this round: the harness recognised its own leases by a runner reference the status view never carries, so scenario 3's dispatch stayed off for an hour after the non-blocking change; fixed and relaunched at 11:08Z without killing its worker.

32. 2026-09-18 11:20–12:00Z, probe round four (a UI-driving lens: headless Chromium through every control as operator, producer and reviewer) and a hands-on pass in the browser pane: a reviewer could not record a reflection verdict from the UI at all, because the wave's in_review action named review.submit with no arguments and no claim step, so the desk sent claimId null (the wave now has a start_review action and bound arguments like the other programs); a second operator's 'Start next step' left a stale state and a standing error after a revision conflict (the control reloads on conflict); /ui/settings/<unknown> rendered an empty pane; the verdict desk enabled Submit for a synopsis the server refuses (line break, heading, identifier); a refused form said 'Tool input failed validation' and dropped the server's path and message; the Paper offered Edit and New section on Methods and Results, which change only through a review, and New section on Problem, whose four sections are fixed; ninety-seven sections titled 't' shared one anchor; a form's save reloaded only its own read, so the Work page head learned of a new cycle ten seconds later (a reload now refreshes every mounted read of the same key); Now listed the viewer's own refusals ('Only this task's producer may submit') as a record's story and now says whose hands it is in; the Feed announced 'An agent joined the project' for every lease beside 'went to an agent'. Later in the same round: a refused sign-in credential showed 'Your session ended' or nothing at all, because the 401 of a credential under check switched the auth mode, which dropped the token and cancelled the check before it could say why (now 'That credential was not accepted.'); an unclaimed review counted as a reader's or the producer's own move; a check with no spaces widened the task page to fifteen thousand pixels; the list's j/k cursor never carried on from the open row; the Work narrowing meant the newest cycle while the head showed the chosen one. Releases 60ebe2d5, 0bf5c836, b67db672, 45005286, 0cba43bd; the UI-driving explorer ran 55 minutes and reported 15 findings, all real. Also: the six request-replay helpers (consolidation, research, reflections, paper, claims, experiments) are one function in contracts (−104 lines); the day's source is still +300 net, bug fixes being mostly checks. Scenario 2's 60M sweep worker held its session for three hours, waited through two rounds of six Thunder boxes, found a sign problem in the plan's own relative-improvement rule (the quantile arm's mean NLL is negative), reported UNDETERMINED with the quantile arm clearly ahead, and passed results review; rates-from-60m is in progress. Spend $84 of $200.

33. 2026-09-18 12:05–12:45Z, regression round: the contract, workflow and sessions lenses rerun on the fixed code. Of round one's findings none reappeared. New: an operator could personally claim and pass the review of a delivery or design its own worker produced (the worker's owner is now excluded from that review like the worker itself; a fresh delegate still may, by design); research.get kept reporting a cycle's creation-time selection after a replan (the record column is immutable by trigger, so the live edges answer) and a byte-identical replan retry answered request_conflict (the domain's replay ledger now covers it); retry_running without a reason invented 'infrastructure failure'; task.submit_delivery accepted another task's brief as evidence; a task with fifty checks and 16,000-character confirmation notes could be delivered but never reviewed (notes are 2,000 characters and a task has at most twenty checks); consolidation's returnTo refusal was 409 where every other domain's is 400; the actor tools' expiry schema said any date-time while requiring milliseconds. Late in the round: a committed task checkpoint retried after the task moved on answered revision_conflict instead of its receipt (the replay now comes first); an invalid automatic lease request named no field. Known gaps left for a workflow version: a consolidation whose extra dependency failed has no edge to end (like research had; consolidation is terminal only at complete), and a research cycle in reflecting draws both 'advance' and 'complete' as ready though only one applies. A worker's nested tool calls inside ui.home are counted as its own in the agent observations; a persistent agent's 'available' list (computed for its source) still shows the review of its own delivery, which it is then refused (candidates carry no producer). Left as is: a junk token that merely starts with ms_ is refused as a session credential on the wrong transport before it is authenticated (checking it first would activate a real session off its transport); the SDK's -32603 on a malformed tools/call; a non-operator's activity page can be shorter than a thousand and still not the end (clients page by cursor, the tool says nothing else). Releases 44d56c18, 08d85161. Scenario 2: posenc-150m-2 abandoned by operator decision after three design rounds (the v2 apparatus has no 4,096-context loader and about $194 of the $200 cap is committed once tokenizer-ab-150m-2's six H100 runs finish); the cycle replanned without it. Scenario 3 closed with awq-70b, dense-32b, specdec-70b and slices-in-repo complete.

34. 2026-09-18 12:50–13:30Z, probe round five (lenses: reviews across every record kind; artifacts, claims, feed, project and code records; reflection waves and consolidation) on fresh beds at HEAD: 22 findings, 18 fixed in four releases (e89cdfe7, 7717a9d1, 08c1fd38, 0a391f8c). The one that mattered most: since the 12:34Z release a leased worker could not submit an experiment design or results at all, because the review excluded the owner and exclusions had to author something in the manifest (an owner who only directed the work authors nothing); exclusions may now name the record owner and the authority that directed the submitting worker, and both are excluded, so an operator who offered a producer's task cannot review its delivery either. Also: the lens Summary check was a backtracking regex (a 150 kB whitespace run held the event loop eleven seconds; 2 MB would have been half an hour) and only saw the first Summary heading at column 0; the experiments package's line-based section reader now serves both, and fenced or commented headings never count. A results review could not pass once another experiment's accepted edits had moved the paper (acceptance now rebases a proposal whose sections are untouched since; a real overlap says to return the work). A reader's newest activity page went blank after a burst of credential administration (the page now walks older until something visible appears). Text made only of zero-width characters passed forty nonblank checks (contracts/text.ts visible() serves them all; the section body too). Long text was clipped mid-surrogate in map briefs, generated artifact titles and generated cycle names (clip()). A review verdict that failed a task left task.failure null; a verdict on a closed review, synthesis during review, synthesis after approval, a stale revision on a reflection review and a consolidation report that was one of its own sources each answered the wrong code. Feed cursors beyond 2^53 were refused with a message that called them not integers. Left as logged: the consolidation with a failed dependency still has no exit (item 33). LOC: the five per-package plain-JSON walkers and two canonical serializers collapsed into contracts plain()/parsed()/canonical() (−270), and twenty-nine event appends into recorded() (−184); the round is net negative in source.

35. 2026-09-18 13:25–14:05Z, probe round six (lenses: the worker lifecycle through real sessions for every record kind; a fresh worker's path rerun; concurrency rerun) on three beds at HEAD: 23 findings, 15 fixed in three releases (e5f773f7, 3e8cb34c, and the one after this entry). One release (ec1753c6) rolled back on production in the deploy's own health check: the reworded planning brief is part of the experiment recipe, which is immutable per version, so it refused to register; the recipes are now version 5 and the published-policies fixture guards recipe hashes as well. Fixed: a wave's owner could review a synthesis its own worker produced, and a consolidation's owner likewise (both now exclude the owner and the directing authority); the lens submit, synthesis submit and wave review actions declared no required input and showed ready, and the wave review showed ready before any claim; a reviewer's review.get {} failed because a one-candidate oneOf argument was never filled; a worker retrying its lost handoff answer was told to refresh metadata (now session_completed); a submit_results preflight ignored the paper change artifact; a non-JSON result was refused as invalid JSON without naming resultFormat; the input_required blocker did not mention the requestId; a session secret refusal named no form; a synopsis accepted bold, links and bullets; a task title or check with a line break rewrote the numbered brief; stale revision and attempt refusals named no current value; offering a lease on ended work said the step took no lease; a halted session left its record leased until an event consumer ran (the lease is released in the halt now). Judged by design or left: a session ended by another hand is recorded as expired (five tests and the runner client encode that status); actor.revoke's claim release still runs through the durable consumer (a 300 ms window); a persistent agent's available list still shows the review of its own delivery; the SDK's -32603 on malformed tools/call. Not reproduced at HEAD (the explorer withdrew them on re-check): revoke appending duplicate events, two current plans, a 330 KB paper blocking assignments.

36. 2026-09-18 14:05–14:45Z, probe round seven on a PostgreSQL bed (a local PostgreSQL 17 cluster behind scripts/ui-demo.ts through MERV_DEMO_CONFIG; production runs PostgreSQL and every earlier bed was SQLite): the records, workflow and concurrency lenses rerun, the ten Postgres-gated test files run for the first time today (all green), and a same-writes diff between a SQLite bed and a Postgres bed (identical answers). Findings that only PostgreSQL shows, all live on production until this round: since 2d3df7ce (2026-09-17 22:57Z) Home's parts shared one read snapshot whose transaction handle lived on the scope object, so every sibling read was a nested_transaction and the page swallowed it: production's Home had drawn without tasks, cycles, workflows, the paper and sessions for fifteen hours (each read now runs in its own async context; a Postgres-gated test covers it); GET /sessions/<id> right after a worker's handoff answered 409 read_only_scope with raw SQL until the sweep, because every GET runs on a snapshot and the poll tried to record the closure (it now reports the closure and leaves the write to the sweep; the integration test polls on Postgres); under a burst of a few hundred writes every pooled connection was held by a writer queued on the state lock, so reads waited behind them and one request in three failed as state_unavailable (reads and snapshots now draw from a reader pool of their own, a writer waits ten seconds, a full pool says state_busy); an agent's status poll scanned every instance for candidates inside a writer transaction (now a snapshot read). Also from this round: a halt that finds a session's handoff already landed halts nothing instead of recording it as halted; a racing copy of a worker's own handoff is told session_completed; a rendered confirmation sheet is not delivery evidence; a plan cannot be attached while prerequisites are unfinished; invisible text is refused in claim statements, experiment prose and the research definition gate; a git session halted before any host attached no longer shows a capture pending forever; four length-only refusal messages say when text was blank; the 16,000-byte plan cap that made scenario 2's planner leave its full plan unattached is 64,000. Accepted as is: GET /sessions and /sessions/status show stored statuses until the one-second sweep records a closure; a revoke's claim release still runs through the durable consumer. LOC: one expiry parser for credentials and keys (−9); a shared command-admission refactor in the workflow engine was tried and reverted because it did not lower the count.

37. 2026-09-18 14:45–16:15Z, probe round eight: the worker-lifecycle and sessions lenses on a fresh PostgreSQL bed at HEAD. Postgres-only, live on production until this round: a worker session's ui.shell failed with raw SQL and its Home was all null, because each nested read recorded an observation and that INSERT was refused inside the read snapshot (observations are skipped inside a read scope; State.readScope says when); a NUL byte in a path id answered 503 state_unavailable (refused as a malformed identifier now). Also: a runner's release of a session whose handoff had already landed overwrote the completed outcome with released, and a release could claim completed for a session that never activated (the handoff is the only proof; release and reconcile share one cheap check of wf_history); an operator who directed a lens worker was not excluded from the synthesis review (each lens lease now records who directed it); a persistent agent's available list offered the review of its own delivery, three lenses running (a lease rule may name the worker actor it excludes, and the engine filters candidates for a named worker); a git experiment's running session was told submit_results was ready when no workspace host was attached (the capture check is part of the preflight); a worker whose record another actor moved was told to refresh and retry into a closed session (it is told the session ended and why); an agent registered with an invisible runner id was permanently unusable; an agent's request id had a hidden 320-character bound blamed on the runner and secret; a pass verdict's preflight ignored the paper acceptance it would perform. Beyond Merv: scenario 2's execute session expired at 16:01Z, one minute before its six 150M runs finished, because a session lives four hours from its last heartbeat and the harness never heartbeats the workers it launches itself (the harness now heartbeats them every ten minutes; the successor worker picked the results up). Accepted as is: a session ended by another actor's move is still recorded as expired (five tests and the runner client encode that status); the SDK's -32603 on malformed tools/call.

38. 2026-09-18 18:14Z, scenario 2 closed. Every record terminal: proxy-harness, proxy-harness-v2 and rates-from-60m done; tokenizer-ab-60m, harness-in-repo and tokenizer-ab-150m-2 complete; tokenizer-ab-150m, posenc-150m and posenc-150m-2 abandoned by operator decision. tokenizer-ab-150m-2 (six H100 runs, exactly 2B observations each, $66.90; project spend $150.98 of the $200 cap, all VMs released) concluded that quantile-binned tokens beat patch embedding at 150M on all three seeds and all three held-out sources by a mean 1.62 nats per scalar: C1 (patch ≥ 5% better at both sizes) is contradicted as written, the pre-registered relative rule is unevaluable (μ[A] ≤ 0) so the primary label is UNDETERMINED while the sign-safe ordering rule says QUANTILE-BETTER, and C2 is undecidable from two sizes (orderings agree, gap ratio 0.975). The results review passed on the first round under the new exclusion rule (the runner-dispatched reviewer was independent of the harness operator who directed the worker). The harness reported DIVERGED on trajectory only (proxy-harness-v2 was observed done where the brief listed in_review as a step; the review happened within the poll interval); 331 executions (329 runner-dispatched, 2 harness-leased). The cycle stays at defining rev 2: its paper problem sections are empty, and reflection (plan Step 9) is an operator decision. All three scenarios are now closed; scenario 1 and 3 reports are from 2026-09-18 ~00:46Z and ~12:00Z.

39. 2026-09-18 18:15–19:40Z, probe round nine: the UI-driving and UI-tools lenses on the PostgreSQL bed (23 findings). The serious ones, all live on production: the Feed column showed the **oldest** thirty posts whenever the newest post's event had fallen out of feed.activity's window, because it derived the head sequence from that window and fell back to `after=0` (feed.list now answers its newest page without a cursor, as activity does, and the page asks for no cursor at all); a post's text silently **deleted** every wf_/review_/art_/claim_ token the page could not name, and activity drew subject-less lines like "is now reflecting" for reflection, lens and consolidation transitions (an unresolved id stays the author's text; a line whose subject cannot be named is not drawn); pinned evidence outside the newest thousand files was invisible on task and review pages, and the map's Files tile silently understated the project (evidence is named by the tail of its id and still opens; the tile marks a floor). Weight: task.list hydrated a full guidance evaluation per row (a 386-task project took 1.4 s per poll and Home three), and the home paper part carried every citation and section in full (1.36 MB after 25 citations within schema limits); guidance belongs to task.get, and the home paper part now carries what the map draws. Production Home went 845 KB → 195 KB → 144 KB across the day. Layout: section prose, file titles, reference names and experiment details did not wrap, so the Paper page was 687,000 px wide at 400 px (four overflow-wrap rules; verified at 400 px headlessly). Also: the Work list's one escape control left the kind narrowing in force and the cycle narrowing could vanish while still filtering; Now counts read 0 while Home was loading; a claim edit did not refresh the card's history; a review reference was named by a raw workflow id; a JSON result accepted integers a double cannot hold, so the pinned exhibit disagreed with its own source; an over-long agent id said an identifier was required. Left as a known gap: task.get and task.list carry each check's text twice (checks and acceptanceChecks, the numbered form agents confirm against), about 2× on a check-heavy project; the Work page polls the whole list every 8 s.
...
40. 2026-09-18 22:00–23:10Z, the fleet run: one project, twenty-seven workers, then interviews. Founder's ask: run many Claude CLI agents in parallel on a single project doing different kinds of work, talk to them afterwards, and find the recurring problems in how their contexts are built and their read tools exposed. Bed: a fresh PostgreSQL bed at HEAD on 3101 (schema `merv_fleet`), the ui-demo project, with a real research wave seeded into it — three tasks, two experiments over three claims, a research cycle, then a reflection wave. Dispatch was the production path: a `MachineRunner` at parallelism 5 with `harness: 'claude'`, its executable a wrapper (`dev_docs/fleet/fleet-claude.zsh`) that passes the runner's own arguments through unchanged, drops `--no-session-persistence` and pins `--session-id`, so every worker could be resumed and interviewed when its lease was over. Twenty-seven launches, 21 analysed in wave one; `dev_docs/fleet/fleet.ts` seeds and drives, `scratchpad/fleet/analyse.py` reads the streams.

    The work was real. Both experiments ran their full grids on this machine — 12 runs at 450,000 steps for the weight-decay sweep, 9 for the training fraction — with finite-difference gradient checks, bit-exact determinism reruns and hash-matched splits and inits. Reviews were not rubber stamps: two designs came back `needs_changes` with specific, correct objections, and the redesigns passed.

    **What the workers do not do.** Across 21 launches and 93 Merv tool calls there were **zero** project-level reads: not one call to `project.get`, `project.records`, `claim.list`, `task.list`, `experiment.list`, `feed.list`, `paper.read`, `research.get`, `artifact.list` or `ui.home` — all of which were listed to them and all of which the engine admits, since `admitRead` has allowed any read tool inside the project since the no-read-constraints ruling. Asked afterwards, 14 of 16 said they had read the tool list inside their frozen assignment as the boundary of what they were allowed to look at ("my execution policy listed a narrow set… so I assumed broader project reads were out of scope for this lease. I also did not try. That was a real gap"), and 11 of 16 made no project-level read at all. The cost is not abstract: three workers on the same project each invented their own network, optimiser, learning rate and step budget for the same claim, whose scope says "same network and task", so their numbers are not comparable; and both experiments landed in a regime where nothing generalises, which a look at a sibling task's delivery would have shown, because the only runs in the project that *did* generalise were sitting in that delivery.

    **The counter-example is inside the codebase.** The reflection recipe (`packages/reflections/src/index.ts:550`) carries a "Live research access" section naming `project.records`, `task.get`, `experiment.get_state`, `paper.read`, `artifact.read` and `review.get`, and its review stage inlines no bytes at all: "Bytes are not included in this context. Inspect them through artifact.read … before judging this evidence." All five lens agents called `project.records` and `paper.read`; the reflection reviewer made 14 `artifact.read` calls and caught a real overclaim in the synthesis. The task and experiment reviewers, whose recipes inline everything (a task review prompt runs 29–47 KB, carrying the producer's own confirmation notes), made **zero** artifact reads across eight reviews and submitted verdicts after one or two calls. Same model, same project, same lease machinery: the difference is the recipe text. `packages/tasks/src/definitions.ts:23` has sections `[task, brief, feedback, checkpoints, checkpointEvidence]` and nothing else.

    **Polling is not a problem.** No launch repeated an identical call more than twice. Re-scanning all three production scenario runs (86 launches, 32 GB of transcripts, `scratchpad/fleet/prod_scan.py`) the longest identical streak is six, and it is `sandboxes.job_status` while waiting for GPU work, which is correct. The most-called production tool is `artifact.read` (1,110 calls) and it is also the most-failed (165), most of them scenario 1's `execution_arguments_forbidden: artifactId is outside the declared execution references` — workers trying to read past their pinned set, before the ruling landed.

    **Shipped** (ebeda46d): the launch preamble in `packages/runner/src/profiles.ts` now says the assignment's tool list binds writes, that reads are open across the project, and that a worker should look for what the project has already fixed before inventing its own; and `review.get`/`review.list` answer `claimable` for their reader, computed from the three clauses `checkStart` applies, so the overview page stops deciding claim eligibility from `producerId` alone and showing "your move" to an excluded contributor who then gets a 403.

    **Surfaced, not fixed.** (a) From `experiment_review` the only routes are `accept_results`, `revise_plan` → planned and `revise_execution` → running (`packages/experiments/src/program.ts:64-66`): a reviewer who wants one sentence of a report reworded must either accept an overclaim or send the whole execution back. It happened in this run — the verdict says "No rerun is needed" and the route it had to take was `returnTo: running`, 12 × 450,000 steps. The reflection workflow has the route this one lacks (`revise_synthesis` keeps the lenses). (b) Review leases are `readOnly`, which also means no shell, so "runs as shown" can only ever mean "looks coherent"; seven of sixteen workers named a sandbox as the one change they wanted. (c) A research cycle sits in `defining` with `nextAction: null` and no assignee until an operator fills the paper's problem/scope/goals/constraints — an autonomous fleet stalls there silently. (d) The durable version of the shipped preamble fix is to give `task.work` and the experiment stages the "Live research access" section the reflection recipe already has, which needs a new recipe version for each.
