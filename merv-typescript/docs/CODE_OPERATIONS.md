...
**Rebinding.** `code.repository.rebind` (a signed-in project administrator, with a `reason` and a `requestId`) binds a hosted project to a different repository identity. It moves no object and does not touch the GitHub repository the project is linked to: `repositoryId` is an opaque Code-side name, Code's own repository is keyed by the project alone, and the whole of what is verified is that that repository holds every commit the project retains as authoritative — main, every accepted commit (a unit's own and every reviewed consolidation round's, which is the class that reaches main), every unit head, every commit a unit's base is pinned to and every resolved base result. A commit it does not hold is named on the refusal (`code_rebind_incomplete`) and nothing is written. A project that was never imported is refused with `code_rebind_unhosted`, and the repository it is already bound to with `code_rebind_unchanged`. `mainOid` is the commit main becomes, recorded as held because the verification proved it; a main that is not ahead of the one being left behind must be named exactly as `acknowledgePreviousMain`, as `code.mirror.retry` asks for a diverged commit. There is no drain and no queue: `code_rebind_busy` refuses, and names, every unresolved base and running project check, every reserved, active or closing writer generation, every session holding a workspace, every unfinished transfer, every unsettled publication and every consolidation holding a frozen candidate set — an unsettled publication because `PublicationHost` is the second writer of main. Verification and an append to the project's `merv-project.json` happen with no transaction open; one transaction then writes the new binding, derives every unpinned unit again and completes the operation, whose `result_json` retains the proof. A crash between them replays: the same `requestId` verifies again and applies once, and a new `requestId` supersedes an unfinished rebind with `code_rebind_superseded`. The binding retains every repository it has been bound to, so acceptances and base pins made under a previous one stay valid and work accepted under it can still be frozen into a later consolidation and published to main; a frozen candidate set never spans a rebind, because one that is still outstanding refuses it. `code.status` shows the lineage, and the prepared operation while it is open. The connection revision is not bumped and nothing is republished — a rebind never touched GitHub.
...
The runner's repository and its private central ref remain machine-local. Legacy task and experiment captures do not advance that central ref or transfer objects between machines. Hosted work uses Code's repository and driver for durable handoff, automatic bases and reviewed merge resolution.
...
A publication is frozen at the repository, base branch and connection revision it was sealed with. Reconnecting GitHub, relinking the repository or turning write automation off fences it: every later reconciliation records `github_conflict` and the consolidation waits in `awaiting_publication` with no action of its own. A 401 for the App JWT or an installation token is `github_app_unavailable` and never discards the owner's stored connection; only a 401 for the owner's own token does that. A consolidation whose publication never froze a binding at all waits the same way, because the binding is only taken on the first successful sync. To end a publication that genuinely cannot reach its repository again, a project administrator or operator key calls `code.publication.release` with its proposalId, a reason and a stable requestId. The release is the whole ending: it returns that consolidation to `consolidating` for another round against the current connection without waiting for a sync, finishes the publication with `code_publication_released` as its last error so the publication list states the ending where the failures were stated, and retains the reviewed facts, the proposal ref and the PR. It is refused with `code_publication_bound` while the repository can still be reached, and refused once the publication has merged; a released publication can no longer be merged. The reason and the released record are retained as a `publication-release` operation in `code_operations`, and the same requestId replays that record.
