...
test('remote discovery and direct calls enforce exact current grants over HTTP and MCP', async (t) => {
  ...
  assert.equal((await httpList(reader.token)).body.tools.length, 80);
  assert.equal((await readerMcp.listTools()).tools.length, 80);
  assert.equal(
  ...
  assert.equal((await otherMcp.listTools()).tools.length, 79);
  assert.equal(
