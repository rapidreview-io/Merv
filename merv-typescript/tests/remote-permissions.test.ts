...
test('remote discovery and direct calls enforce exact current grants over HTTP and MCP', async (t) => {
  ...
  assert.equal((await otherMcp.listTools()).tools.length, 76);
  assert.equal(
  ...
  app.ctx.scope.toolPolicy.replace([]);
  assert.equal((await readerMcp.listTools()).tools.length, 76);
  assert.equal((await httpList(reader.token)).body.tools.length, 76);
