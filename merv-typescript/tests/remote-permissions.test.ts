...
test('remote discovery and direct calls enforce exact current grants over HTTP and MCP', async (t) => {
  ...
  assert.equal((await httpList(reader.token)).body.tools.length, 72);
  assert.equal((await readerMcp.listTools()).tools.length, 72);
  assert.equal(
  ...
  assert.equal((await otherMcp.listTools()).tools.length, 71);
  assert.equal(
  ...
  app.ctx.scope.toolPolicy.replace([]);
  assert.equal((await readerMcp.listTools()).tools.length, 71);
  assert.equal((await httpList(reader.token)).body.tools.length, 71);
