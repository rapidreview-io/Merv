...
for (const backend of backends) {
  ...
  for (const point of [
    ...
    test(
      `${backend}: an upload whose process ends ${point} is finished by the next one, exactly once`,
      optional(backend),
      async (t) => {
        const f = await fixture(t, backend);
        await f.lease('ses_1');
  ...
  test(
    `${backend}: a lease reserves the next generation only once the last one closed`,
    optional(backend),
    async (t) => {
      const f = await fixture(t, backend, 0);
  ...
  test(
    `${backend}: uploads advance a unit's branch only under the whole fence, and the final capture ends the generation exactly once`,
    optional(backend),
    async (t) => {
      const f = await fixture(t, backend);
      await f.lease('ses_1');
      await f.event('session.workspace_attached', 'ses_1');
      const first = f.source.commit({ 'a.txt': 'one\n' }, 'first');
      const done = await f.upload(
        'checkpoint',
        'ses_1',
        1,
        f.root,
        f.source.bundle(first, [f.root]),
      );
      ...
      f.end('ses_1');
      await assert.rejects(
        f.begin('checkpoint', 'ses_1', 1, first, next),
  ...
  test(
    `${backend}: a later upload ends one that was only receiving, and a commit succeeds only once admitted`,
    optional(backend),
    async (t) => {
      const f = await fixture(t, backend);
      await f.lease('ses_1');
  ...
  test(
    `${backend}: a quarantined final capture blocks the unit until a signed-in administrator fences it`,
    optional(backend),
    async (t) => {
      const f = await fixture(t, backend);
      await f.lease('ses_1');
  ...
  test(
    `${backend}: after a fence the old generation is stale and what it was sending is held, never admitted`,
    optional(backend),
    async (t) => {
      const f = await fixture(t, backend);
      await f.lease('ses_1');
