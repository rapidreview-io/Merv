 ...
 */
async function fixture(t: TestContext, backend: Backend, connected = false) {
...
for (const backend of backends) {
  test(
    `${backend}: a publishing unit takes its base from main as well as its dependencies`,
    optional(backend),
    async (t) => {
  ...
  test(
    `${backend}: a clash with main is a resolution task like any other conflict`,
    optional(backend),
    async (t) => {
      const f = await fixture(t, backend);
      const dependency = await f.declare('Dependency');
  ...
  test(
    `${backend}: publishing is declared once, before the work stands anywhere, and only on hosted work`,
    optional(backend),
    async (t) => {
      const f = await fixture(t, backend);
      ...
      await assert.rejects(
        ...
        backend === 'sqlite' ? /declared once/ : { code: /^state_/ },
      );
  ...
  test(
    `${backend}: acceptedSince names the accepted work main does not hold, and nothing else`,
    optional(backend),
    async (t) => {
      const f = await fixture(t, backend);
  ...
  test(
    `${backend}: a quarantine never reaches work that has ended, but a publication wait does`,
    optional(backend),
    async (t) => {
      const f = await fixture(t, backend);
  ...
  test(
    `${backend}: an acceptance that cannot be sealed is still recorded, and says so`,
    optional(backend),
    async (t) => {
      const f = await fixture(t, backend);
  ...
  test(
    `${backend}: an accepted publishing unit waits on one pull request and then carries main`,
    optional(backend),
    async (t) => {
      ...
      assert.equal(
        git(f.bare, ['rev-parse', `refs/merv/proposals/${opened.proposalId}`]),
        f.feature,
      );
      assert.equal(
  ...
  test(
    `${backend}: main moving under an approved head is a stale wait, not a failure`,
    optional(backend),
    async (t) => {
      const f = await fixture(t, backend, true);
      await f.canary();
  ...
  test(
    `${backend}: a disabled project and a published tree mismatch both show on the unit`,
    optional(backend),
    async (t) => {
      const f = await fixture(t, backend, true);
      await f.canary();
  ...
  test(
    `${backend}: with Code unloaded nothing about publication can be asked or declared`,
    optional(backend),
    async (t) => {
      const f = await fixture(t, backend);
      const work = await f.declare('Publishing');
