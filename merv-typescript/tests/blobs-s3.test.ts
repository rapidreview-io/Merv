...
async function copyFixture(t: TestContext) {
  ...
  const bytes = Buffer.alloc(2_000_001, 120);
  const keyHash = createHash('sha256').update(bytes).digest('hex');
  const sourceKey = `legacy/project_1/${keyHash}`;
  const destinationKey = `evidence/v1/project_1/${keyHash}`;
  f.server.objects.set(sourceKey, bytes);
  return { ...f, source, bytes, keyHash, sourceKey, destinationKey };
}

test('verified S3 copy streams large hashes and uses immutable source and R2 destination conditions', async (t) => {
  const f = await copyFixture(t);
  const transfer = () =>
    f.blobs.copyVerifiedFrom(f.source, 'project_1', f.keyHash, f.bytes.length, {
      destinationCondition: 'r2',
    });
  assert.deepEqual(await transfer(), { hash: f.keyHash, size: f.bytes.length });
  assert.deepEqual(f.server.objects.get(f.destinationKey), f.bytes);
  const copy = f.server.requests.find((r) => r.method === 'PUT')!;
  assert.equal(copy.body.length, 0, 'Server-side copy does not upload bytes through the process');
  assert.equal(copy.headers['x-amz-copy-source'], `merv-artifacts/${f.sourceKey}`);
  assert.equal(
    copy.headers['x-amz-copy-source-if-match'],
    `"${createHash('md5').update(f.bytes).digest('hex')}"`,
  );
  assert.equal(copy.headers['if-none-match'], undefined);
  assert.equal(copy.headers['cf-copy-destination-if-none-match'], '*');
  assert.match(copy.headers.authorization!, /cf-copy-destination-if-none-match/);
  assert.deepEqual(
    f.server.requests.map((r) => r.method),
    ['HEAD', 'GET', 'PUT', 'HEAD', 'GET'],
  );
  assert.deepEqual(
    await transfer(),
    { hash: f.keyHash, size: f.bytes.length },
    '412 only succeeds after a complete destination check',
  );
  assert.deepEqual(f.server.objects.get(f.sourceKey), f.bytes);
  f.server.objects.set(f.destinationKey, Buffer.alloc(f.bytes.length, 121));
  await assert.rejects(transfer(), code('blob_corrupt'));
  assert.equal(
    f.server.objects.get(f.destinationKey)![0],
    121,
    'Corrupt destinations are not overwritten',
  );
});

test('verified S3 copy rejects changed source bytes, bad metadata and oversized transfers', async (t) => {
  const f = await copyFixture(t);
  const held = f.server.holdNext('PUT');
  const pending = f.blobs.copyVerifiedFrom(f.source, 'project_1', f.keyHash, f.bytes.length);
  const rejected = assert.rejects(pending, code('blob_not_found'));
  await held.started;
  f.server.objects.set(f.sourceKey, Buffer.alloc(f.bytes.length, 121));
  held.release();
  await rejected;
  assert.equal(f.server.objects.has(f.destinationKey), false);
  await assert.rejects(
    f.blobs.copyVerifiedFrom(f.source, 'project_1', f.keyHash, f.bytes.length),
    code('blob_corrupt'),
  );
  ...
  assert.throws(
    () => f.blobs.copyVerifiedFrom(f.source, 'project_1', f.keyHash, 512 * 1024 * 1024 + 1),
    code('blob_size'),
  );
  assert.equal(f.server.requests.length, count);
  f.server.objects.set(f.sourceKey, f.bytes);
  await assert.rejects(
    f.blobs.copyVerifiedFrom(f.source, 'project_1', f.keyHash, f.bytes.length - 1),
    code('blob_corrupt'),
  );
});

test('verified S3 copy drains both providers before either client closes', async (t) => {
  const f = await copyFixture(t);
  const held = f.server.holdNext('PUT');
  const pending = f.blobs.copyVerifiedFrom(f.source, 'project_1', f.keyHash, f.bytes.length);
  await held.started;
  assert.equal(f.server.requests.find((r) => r.method === 'PUT')!.headers['if-none-match'], '*');
  let sourceClosed = false,
    destinationClosed = false;
  const sourceClosing = f.source.close().then(() => {
    sourceClosed = true;
  });
  const destinationClosing = f.blobs.close().then(() => {
    destinationClosed = true;
  });
  await assert.rejects(f.source.get('project_1', f.keyHash), code('blobs_closed'));
  await assert.rejects(f.blobs.get('project_1', f.keyHash), code('blobs_closed'));
  assert.equal(sourceClosed || destinationClosed, false);
  held.release();
  await pending;
  await Promise.all([sourceClosing, destinationClosing]);
  assert.equal(sourceClosed && destinationClosed, true);
});

test('offline S3 media copy discovers a missing size from HEAD and still verifies both streamed hashes', async (t) => {
  const f = await copyFixture(t);
  assert.deepEqual(await f.blobs.copyVerifiedFrom(f.source, 'project_1', f.keyHash), {
    hash: f.keyHash,
    size: f.bytes.length,
  });
  assert.deepEqual(await f.blobs.copyVerifiedFrom(f.source, 'project_1', f.keyHash), {
  ...
  f.server.objects.set(f.sourceKey, Buffer.alloc(f.bytes.length, 99));
  await assert.rejects(
    f.blobs.copyVerifiedFrom(f.source, 'project_1', f.keyHash),
    code('blob_corrupt'),
  );
  f.server.objects.delete(f.sourceKey);
  await assert.rejects(
    f.blobs.copyVerifiedFrom(f.source, 'project_1', f.keyHash),
    code('blob_not_found'),
  );
...
test('retained zero-byte media verifies SHA256(empty), exact HEAD length and conditional copy', async (t) => {
  const f = await copyFixture(t);
  const empty = Buffer.alloc(0);
  const emptyHash = createHash('sha256').update(empty).digest('hex');
  f.server.objects.set(`legacy/project_1/${emptyHash}`, empty);
  assert.deepEqual(await f.blobs.copyVerifiedFrom(f.source, 'project_1', emptyHash, 0), {
    hash: emptyHash,
    size: 0,
  });
  assert.deepEqual(await f.blobs.copyVerifiedFrom(f.source, 'project_1', emptyHash), {
    hash: emptyHash,
    size: 0,
  });
  const link = await f.blobs.download('project_1', emptyHash, 0);
  const response = await fetch(link.url);
  assert.equal(response.status, 200);
  assert.equal(response.headers.get('content-length'), '0');
  assert.equal((await response.arrayBuffer()).byteLength, 0);
  f.server.objects.set(f.sourceKey, empty);
  await assert.rejects(
    f.blobs.copyVerifiedFrom(f.source, 'project_1', f.keyHash, 0),
    code('blob_corrupt'),
  );
  await assert.rejects(
    f.blobs.copyVerifiedFrom(f.source, 'project_1', emptyHash, 1),
    code('blob_corrupt'),
  );
});
