...
test('component migrations are independent, immutable, atomic and persistent', (t) => {
  ...
  state.migrate('sample', a);
  state.migrate('sample', a);
  state.migrate('other', [{ version: 1, sql: 'CREATE TABLE other(id TEXT PRIMARY KEY);' }]);
  assert.throws(
    () => state.migrate('sample', [{ version: 1, sql: 'CREATE TABLE changed(id TEXT);' }]),
    /changed/,
  );
  assert.throws(() =>
    state.migrate('failure', [
      { version: 1, sql: 'CREATE TABLE undone(id TEXT); INSERT INTO missing VALUES(1);' },
    ]),
  );
  assert.equal(
    state.read((sql) => sql.get("SELECT name FROM sqlite_master WHERE name='undone'")),
    undefined,
  );
  ...
  assert.throws(
    () =>
      state.transaction((current) => {
        tx = current;
        current.run('INSERT INTO sample VALUES(?)', 'rollback');
  ...
  assert.equal(
    state.read((sql) => sql.get('SELECT * FROM sample')),
    undefined,
  );
  assert.throws(
    () =>
  ...
  assert.throws(
    () => state.transaction(() => Promise.reject(new Error('contained rejection'))),
    /synchronous/,
  );
  ...
  try {
    assert.equal(
      second.read((sql) => sql.get<{ id: string }>('SELECT id FROM sample'))?.id,
      'retained',
    );
  } finally {
    second.close();
...
test('events commit with the transaction and survive reopening', (t) => {
  ...
  assert.throws(() =>
    state.transaction((tx) => {
      state.appendEvent(tx, { ...caller, type: 'rollback', subjectId: 'x', data: {} });
      throw new Error('abort');
    }),
  );
  assert.equal(state.events(caller.projectId).length, before);
  state.transaction((tx) =>
    state.appendEvent(tx, { ...caller, type: 'committed', subjectId: 'y', data: { number: 1 } }),
  );
  const second = new SqliteState(join(dir, 'state.sqlite'));
  try {
    assert.equal(second.events(caller.projectId).at(-1)?.type, 'committed');
  } finally {
    second.close();
  }
});
test('credentials enforce roles, project boundaries and revocation', (t) => {
  ...
  scope.require(rc, 'review');
  assert.throws(() => scope.require(rc, 'write'), /lacks write/);
  const other = scope.bootstrap({ projectName: 'Second project', actorName: 'Other' });
  assert.throws(
    () => scope.require({ ...caller, projectId: other.project.id }, 'read'),
    /cannot access/,
  );
...
test('artifacts retain exact bytes, reject mutation, scope reads and detect corruption', (t) => {
  ...
  assert.equal(
    artifacts.create(caller, { title: 'Same content', content: 'retained content' }).hash,
    value.hash,
  );
  assert.throws(
    () =>
      state.transaction((tx) =>
        tx.run('UPDATE artifacts SET title=? WHERE id=?', 'changed', value.id),
      ),
    /immutable/,
  );
  assert.throws(
  ...
  assert.throws(
    () => artifacts.get({ actorId: other.actor.id, projectId: other.project.id }, value.id),
    /not found/,
  );
  assert.throws(() => blobs.get('../escape', value.hash), /namespace/);
  assert.throws(
    () => artifacts.create(caller, { title: 'Bad', content: '!!!', encoding: 'base64' }),
    /base64/,
  );
  const binary = artifacts.create(caller, {
    title: 'Invalid UTF8',
    content: '/w==',
    encoding: 'base64',
    mediaType: 'text/plain',
  });
  assert.deepEqual(artifacts.read(caller, binary.id), {
    artifact: binary,
    content: '/w==',
    encoding: 'base64',
  });
