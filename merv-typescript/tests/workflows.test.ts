...
test('durable graph transitions record exact command responses, history, and events once', (t) => {
  ...
  const finished = workflows.transition(caller, {
    instanceId: initial.id,
    expectedRevision: 1,
    action: 'accept',
    requestId: 'accept-1',
  ...
  assert.equal(
    state.events(caller.projectId).filter((event) => event.type === 'workflow.transition').length,
    3,
  );
  assert.throws(
    () => workflows.transition(caller, { ...command, requestId: 'new', expectedRevision: 2 }),
    code('invalid_transition'),
  );
  assert.throws(
    () => workflows.transition(caller, { ...command, data: { changed: true } }),
    code('request_conflict'),
  );
  assert.throws(
    () => workflows.start(caller, { ...start, data: { title: 'Different' } }),
    code('request_conflict'),
  );
  assert.throws(
    () => workflows.transition(caller, { ...command, requestId: 'start-1' }),
    code('request_conflict'),
  );
});
...
test('versions are pinned and changed declarations are rejected across restart', (t) => {
  ...
  assert.equal(
    first.workflows.start(first.caller, { workflow: 'approval', requestId: 'start-v2' }).version,
    2,
  );
  first.state.close();
  ...
  assert.equal(
    workflows.transition(first.caller, {
      instanceId: initial.id,
      action: 'submit',
      expectedRevision: 0,
  ...
  assert.deepEqual(
    workflows.start(first.caller, { workflow: 'approval', requestId: 'start' }),
    initial,
  );
...
test('competing connections reject stale revisions and transaction rollback includes ledger and events', (t) => {
  ...
  assert.throws(
    () => state2.transaction((tx) => workflows.get(caller, initial.id, tx)),
...
test('all project reads and mutation replays check actor scope and permission', (t) => {
  ...
  assert.throws(
    () => workflows.get({ ...other, projectId: caller.projectId }, initial.id),
    code('forbidden'),
  );
  ...
  assert.throws(
    () => workflows.start(readerCaller, { workflow: 'approval', requestId: 'start' }),
    code('forbidden'),
  );
  ...
  assert.throws(
    () => workflows.list({ actorId: producer.id, projectId: caller.projectId }),
    code('forbidden'),
  );
});
...
test('managed program handles prevent bypass and disposal preserves durable data', (t) => {
  ...
  assert.throws(
    () => workflows.start(caller, { workflow: 'owned', requestId: 'bypass-start' }),
  ...
  assert.throws(
    () =>
      program.transition(caller, {
        instanceId: workflows.start(caller, { workflow: 'approval', requestId: 'other-start' }).id,
        action: 'submit',
        expectedRevision: 0,
  ...
  const accepted = program.transition(reviewerCaller, {
    instanceId: initial.id,
    action: 'accept',
    expectedRevision: 1,
  ...
  assert.throws(
    () => program.start(caller, { workflow: 'owned', requestId: 'disposed' }),
    code('workflow_unavailable'),
  );
  const replacement = workflows.register(definition);
  assert.deepEqual(
    replacement.start(caller, { workflow: 'owned', requestId: 'owned-start' }),
    initial,
  );
...
test('graph validation and defensive copies prevent changing installed behavior', (t) => {
  ...
  assert.throws(
    () => workflows.register({ ...graph(), name: 'invalid', terminal: ['missing'] }),
    /terminal states/,
  );
  assert.throws(
    () =>
  ...
  assert.throws(
    () => workflows.start(caller, { workflow: 'approval', requestId: '', data: {} }),
    code('invalid_request'),
  );
  assert.throws(
    () => workflows.start(caller, { workflow: 'approval', requestId: 'nan', data: { value: NaN } }),
    /finite JSON/,
  );
});
