...
test('service reservations and settlements survive every transactional boundary exactly once without Code', async (t) => {
...
test('simultaneous service records share one project cap; expired work releases it and rejects late settlement', async (t) => {
  const f = await fixture(t);
...
test('shared wall time is charged once per project and fully to immutable roots; budgets and dispatch refuse without reservations', async (t) => {
  const f = await fixture(t);
