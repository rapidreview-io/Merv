...
test('assembled Cordis application completes MCP task review across two full restarts', async () => {
  ...
  try {
    ...
    const p = app.ctx.scope.issueActor(caller, { name: 'Producer', role: 'producer' }),
      r = app.ctx.scope.issueActor(caller, { name: 'Reviewer', role: 'reviewer' });
    ...
    const submitted = await call(producer, 'task.submit_delivery', {
      taskId: task.id,
    ...
    assert.equal(
      app.ctx.state.events(caller.projectId).filter((e) => e.type === 'task.review_applied').length,
      1,
    );
    assert.equal(
      app.ctx.artifacts.read(caller, delivery.id).content,
      'Sum is 20. Calculation: 2+4+6+8=20.',
    );
  } finally {
    await producer?.close();
