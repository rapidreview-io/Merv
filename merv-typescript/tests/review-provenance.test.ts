...
for (const backend of backends) {
  test(
    `${backend}: provenance migration preserves populated reviews and freezes certificates`,
    optional(backend),
    async (t) => {
      const f = await resolutionFixture(t, backend, { reviews: 8 });
      const producer = {
        projectId: f.admin.projectId,
        actorId: (await f.scope.issueActor(f.admin, { name: 'Producer', role: 'producer' })).actor
          .id,
      };
      ...
      for (const id of [before.id, certified.id]) {
        ...
        await assert.rejects(
          ...
          backend === 'sqlite' ? /durable/ : { code: /^state_/ },
        );
      }
      release();
  ...
  test(
    `${backend}: submission checks the caller's current directing authority against pinned provenance`,
    optional(backend),
    async (t) => {
      const f = await resolutionFixture(t, backend);
      const producer = {
        projectId: f.admin.projectId,
        actorId: (await f.scope.issueActor(f.admin, { name: 'Contributor', role: 'producer' }))
          .actor.id,
      };
      const output = await f.artifacts.create(producer, { title: 'Evidence', content: 'Checked' });
      const contributor = {
        projectId: f.admin.projectId,
        actorId: (await f.scope.issueActor(f.admin, { name: 'Input writer', role: 'producer' }))
          .actor.id,
      };
      const body = {
        formatVersion: 1 as const,
