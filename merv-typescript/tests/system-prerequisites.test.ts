...
for (const backend of backends)
  test(
    `${backend}: provider prerequisites gate work without declaring failure`,
    optional(backend),
    async (t) => {
      ...
      assert.equal(
        (await f.workflows.dependencies(f.admin, waiter.id)).dependencies[0].kind,
        'system',
      );
      await f.state.transaction((tx) =>
...
for (const backend of backends)
  test(
    `${backend}: workspace-free tasks work with Code absent and task manifests retain their published hashes`,
    optional(backend),
    async (t) => {
      const f = await resolutionFixture(t, backend);
