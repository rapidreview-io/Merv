...
test('a passing check seals the base with a receipt, and the machine and its source go back', async (t) => {
...
test('a failing check drops the auto-merge ref, so the one resolution task can seal it', async (t) => {
  const f = await baseFixture(t);
  ...
  await f.state.transaction((tx) => f.bases.linkTask(tx, f.projectId, base.key, 'task_resolution'));
  await f.state.transaction(async (tx) =>
...
test('a check holds its own reservation and never the project’s merge lane', async (t) => {
  const f = await baseFixture(t);
...
test('a suspended check gives its machine back, and a quarantine keeps a recorded verdict', async (t) => {
  const f = await baseFixture(t);
...
test('the database refuses an unpaired verdict, a second verdict and an unresolved seal', async (t) => {
  const f = await baseFixture(t);
  ...
  // PostgreSQL does not disclose which constraint refused, so what this proves is that each
  // of the three is refused by the database and not by a service.
  const said = (_rule: RegExp) => ({ code: 'state_constraint' });
...
test('a configured command with no adapter leaves the base unsealed for an operator', async (t) => {
  const f = await baseFixture(t);
  await configure(f, SPEC);
  const { a, b } = f.commits;
  const base = await f.state.transaction((tx) => f.bases.ensure(tx, f.projectId, [a, b]));
...
test('a timeout is a verdict, and a job with no result is infrastructure', async (t) => {
  const f = await baseFixture(t);
...
test('an interrupted first step rents nothing twice, and no command means no machine', async (t) => {
  const f = await baseFixture(t);
  const adapter = scripted([ran(0)]);
  f.bases.checks = adapter.checks;
  await configure(f, SPEC);
  ...
  await g.state.transaction((tx) => g.bases.ensure(tx, g.projectId, [g.commits.a, g.commits.b]));
  await settle(g, 2);
