...
function fixture(t: TestContext) {
  ...
  const artifacts = new ArtifactStore(state, scope, blobs),
    feed = new FeedService(state, scope, artifacts);
...
test('standalone Feed admits reviewers, scopes communication, and validates artifact attachments', (t) => {
  const f = fixture(t);
  const evidence = f.artifacts.create(f.producer, {
    title: 'Evidence',
    content: 'Immutable evidence.',
  });
  const post = f.feed.post(f.reviewer, {
  ...
  assert.throws(
    () => f.feed.post(f.reader, { body: 'Cannot post.', requestId: 'reader' }),
    code('forbidden'),
  );
  assert.throws(
    () =>
      f.artifacts.create(f.reviewer, {
        title: 'Not allowed',
  ...
  const foreignEvidence = f.artifacts.create(foreign, {
    title: 'Foreign',
    content: 'Another project.',
  });
  assert.throws(
    () =>
      f.feed.post(f.producer, {
  ...
  assert.throws(
    () => f.feed.list({ ...f.reviewer, projectId: foreign.projectId }),
    code('forbidden'),
  );
  assert.throws(
    () =>
  ...
  assert.throws(
    () =>
      f.feed.post(f.producer, { body: 'Missing', artifactIds: ['missing'], requestId: 'missing' }),
    code('not_found'),
  );
...
test('Feed preserves exact content, deduplicates per actor, and rechecks permission on retries', (t) => {
  ...
  assert.equal(f.feed.list(f.producer).length, 1);
  assert.equal(
    f.feed.activity(f.producer).filter((event) => event.type === 'feed.posted').length,
    1,
  );
  assert.throws(
    () => f.feed.post(f.producer, { ...input, body: 'Changed.' }),
    code('request_conflict'),
  );
  assert.throws(
    () => f.feed.post(f.producer, { ...input, artifactIds: [] }),
    code('request_conflict'),
  );
...
test('Feed validates text, direct-call attachment shapes, and cursor limits', (t) => {
  ...
  for (const body of ['', ' \n\t ', 'x'.repeat(8001)]) {
    assert.throws(
      () => f.feed.post(f.producer, { body, requestId: 'invalid-body' }),
      code('invalid_body'),
    );
  }
  ...
  assert.throws(
    () => f.feed.post(f.producer, { body: 'Good', requestId: ' ' }),
    code('invalid_request'),
  );
  assert.throws(
    () => f.feed.post(f.producer, { body: 'Good', requestId: 'x'.repeat(201) }),
    code('invalid_request'),
  );
  assert.throws(
    () =>
  ...
  assert.throws(
    () => f.feed.post(f.producer, { body: 'Good', requestId: 'bad-item', artifactIds: [''] }),
    code('invalid_attachments'),
  );
  for (const input of [
    { after: -1 },
...
test('Feed post, durable event, and request record roll back together and retry safely', (t) => {
  ...
  assert.equal(
    f.state.read((sql) => sql.get<{ count: number }>('SELECT COUNT(*) AS count FROM feed_requests'))
      ?.count,
    0,
  );
...
test('Feed posts and deduplication records are immutable in SQLite', (t) => {
  ...
  assert.throws(
    () =>
      f.state.transaction((tx) =>
        tx.run('UPDATE feed_posts SET body = ? WHERE id = ?', 'Edited', post.id),
      ),
    /immutable/,
  );
  assert.throws(
    () => f.state.transaction((tx) => tx.run('DELETE FROM feed_posts WHERE id = ?', post.id)),
    /immutable/,
  );
  assert.throws(
    () =>
  ...
  assert.throws(
    () =>
      f.state.transaction((tx) => tx.run('UPDATE feed_requests SET input_hash = ?', 'overwritten')),
    /immutable/,
  );
...
test('Feed cursor pages and activity preserve project boundaries and survive reopening', (t) => {
  ...
  const records = Array.from({ length: 55 }, (_, i) =>
    f.feed.post(f.producer, { body: `Message ${i}`, requestId: `page-${i}` }),
  );
  const other = f.scope.bootstrap({ projectName: 'Other project', actorName: 'Other operator' });
  const foreign: Caller = { actorId: other.actor.id, projectId: other.project.id };
  const foreignPost = f.feed.post(foreign, {
    body: 'Hidden from the first project.',
    requestId: 'foreign',
  });
  const final = f.feed.post(f.reviewer, {
  ...
  assert.deepEqual(
    f.feed.activity(f.reader, cursor),
  ...
  try {
    ...
    assert.deepEqual(
      feed.post(f.reviewer, { body: 'After another project’s post.', requestId: 'last' }),
      final,
    );
  } finally {
    reopened.close();
...
test('Feed activity exposes actor administration metadata only to project operators', (t) => {
  ...
  assert.equal(
    f.feed.activity(f.reader).some((event) => event.subjectId === revoked.actor.id),
    false,
  );
  assert.ok(
    f.feed
      .activity(f.operator)
      .some((event) => event.type === 'actor.revoked' && event.subjectId === revoked.actor.id),
  );
});
