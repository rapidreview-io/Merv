...
function storage(t: TestContext, backend: Backend) {
  const directory = mkdtempSync(join(tmpdir(), 'merv-claims-retirement-'));
  const schema = `claims_retirement_${randomUUID().replaceAll('-', '')}`;
  const env = `MERV_CLAIMS_RETIREMENT_${randomUUID().replaceAll('-', '').toUpperCase()}`;
  if (backend === 'postgres') process.env[env] = postgresUrl;
...
const table = async (sql: Sql, name: string) =>
  sql.dialect === 'postgres'
    ...
    : Boolean(await sql.get("SELECT name FROM sqlite_master WHERE type='table' AND name=?", name));
...
for (const backend of backends)
  test(
    `retired claims become Markdown artifacts and their tables go, events stay, on ${backend}`,
    { skip: backend === 'postgres' && !postgresUrl },
    async (t) => {
      const store = storage(t, backend);
      ...
      assert.deepEqual(
        ...
        [
          claimArtifactId(owner.projectId, edited.id),
          claimArtifactId(owner.projectId, long.id),
        ].sort(),
      );
      ...
      const after = await app.ctx.state.read(async (sql) => ({
        ...
        migrations: await sql.all(
          "SELECT version FROM component_migrations WHERE component='claims'",
        ),
        events: (
...
for (const backend of backends)
  test(
    `a database that never held claims is left as it was on ${backend}`,
    { skip: backend === 'postgres' && !postgresUrl },
    async (t) => {
      const store = storage(t, backend);
      const app = await store.boot();
...
test(
  'a server that reads while another is dropping the claims tables leaves the work to it',
  { skip: !postgresUrl },
  async (t) => {
    const store = storage(t, 'postgres');
    const state = await store.open();
    await state.migrate('claims', [claimsV1]);
    await state.transaction(async (tx) => {
      await tx.run(
