...
test('Code receipts survive concurrent replay, isolate principals and roll back with their mutation', async (t) => {
  ...
  assert.equal((await receipts()).length, 2, 'another principal owns its own request namespace');
  assert.deepEqual(
