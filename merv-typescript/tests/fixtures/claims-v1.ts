...
/**
 * The published claims v1 migration, its PostgreSQL text copied verbatim from the retired Claims
 * plugin (packages/claims/src/index.postgres.ts before its removal), so a test can build the
 * tables a deployed database still holds. Production applied it under hash da705fe4…
 * (tests/fixtures/published-migrations.json); keep it byte-identical.
 */
