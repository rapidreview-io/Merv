 ...
 */
async function hosted(t: TestContext, backend: Backend, fault?: (point: FaultPoint) => void) {
...
for (const backend of backends) {
  test(
    `${backend}: a rebind proves Code holds the history, retains what it leaves and keeps the acceptances made under it`,
    optional(backend),
    async (t) => {
      const h = await hosted(t, backend);
  ...
  test(
    `${backend}: a rebind refuses what it cannot prove, what it would not change and the main it would drop`,
    optional(backend),
    async (t) => {
      const h = await hosted(t, backend);
  ...
  test(
    `${backend}: the proof covers base pins and the acceptances of reviewed consolidation rounds`,
    optional(backend),
    async (t) => {
      const h = await hosted(t, backend);
  ...
  test(
    `${backend}: work in flight is named and refuses the rebind, and publication is neither fenced nor released`,
    optional(backend),
    async (t) => {
      const h = await hosted(t, backend);
  ...
  test(
    `${backend}: an unfinished rebind is superseded rather than standing in the way`,
    optional(backend),
    async (t) => {
      const h = await hosted(t, backend);
      const unit = await h.unit('unit');
  ...
  test(
    `${backend}: the marker is appended before the transaction, so a crash between them replays`,
    optional(backend),
    async (t) => {
      const h = await hosted(t, backend, faultAt('after_rebind_marker'));
      const listed = { format: 2, projectId: h.human.projectId, repositoryIds: [OLD, NEW] };
  ...
  test(
    `${backend}: the binding trigger admits only the write its own prepared operation names`,
    optional(backend),
    async (t) => {
      const h = await hosted(t, backend);
  ...
  test(
    `${backend}: the marker says which identities a directory serves, and a rebind clears the memo`,
    optional(backend),
    async (t) => {
      const h = await hosted(t, backend);
