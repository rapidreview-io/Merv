...
const runDirectory=resolve(process.argv[2] ?? join('live-runs',new Date().toISOString().replaceAll(':','-')))
...
interface ObservedCall { tool:string; status:string; errorCode?:string; transportError?:string }
const summary:{phase:Phase;threadId:string;exitCode:number;toolCalls:number;calls:ObservedCall[]}[]=[]
const readTools=['actor.whoami','project.get','task.get','task.list','review.get','review.list','artifact.get','artifact.read','artifact.list','workflow.get','workflow.history']
...
async function codex(phase:Phase,token:string,url:string,prompt:string) {
  ...
  const args=['exec','--ignore-user-config','--ephemeral','--skip-git-repo-check','--sandbox','read-only','--json','--color','never','-C',workingDirectory,
    '-c','approval_policy="never"','-c','features.apps=false','-c','features.shell_tool=false',
    '-c',`mcp_servers.merv_typescript.url=${JSON.stringify(url+'/mcp')}`,
    '-c','mcp_servers.merv_typescript.bearer_token_env_var="MERV_TEST_TOKEN"',
    '-c','mcp_servers.merv_typescript.required=true',
    '-c',`mcp_servers.merv_typescript.enabled_tools=${JSON.stringify([...readTools,...phaseWrites[phase]])}`,
    ...
    '-c',`mcp_servers.merv_typescript.tools={${phaseWrites[phase].map(name=>`${JSON.stringify(name)}={approval_mode="approve"}`).join(',')}}`,
    '-o',join(runDirectory,`${phase}.final.txt`),'-']
  ...
  const child=spawn(process.env.MERV_CODEX_BIN ?? 'codex',args,{env:childEnv,stdio:['pipe','pipe','pipe']})
      ...
      try {
        ...
        if(event.type==='item.completed' && event.item?.type==='mcp_tool_call') {
          ...
          for(const content of item.result?.content ?? []) {
            if(content.type==='text')try {errorCode=JSON.parse(content.text)?.error?.code ?? errorCode}catch{/* Text output need not be JSON. */}
          }
          calls.push({tool:item.tool,status:item.status,...(errorCode?{errorCode}:{}),...(item.error?{transportError:item.error.message}:{})})
          console.log(JSON.stringify({phase,...calls.at(-1)}))
        }
      } catch {/* Keep raw output for debugging. */}
    }
  })
  child.stderr.pipe(errors)
  child.stdin.end(`You are testing the new Merv TypeScript application. Use ONLY the merv_typescript MCP tools to interact with it. Do not inspect the database or server source, do not edit any files, and do not invoke shell commands. Treat tool failures as test observations; report them accurately.\n\n${prompt}\n\nEnd with a concise account of what you actually verified and the task/review IDs.`)
  const timeout=setTimeout(()=>child.kill('SIGTERM'),600_000)
  const exitCode=await new Promise<number>((resolve,reject)=>{child.once('error',reject);child.once('close',code=>resolve(code ?? -1))}).finally(async()=>{clearTimeout(timeout);output.end();errors.end();await Promise.all([finished(output),finished(errors)])})
  ...
  const required:Record<Phase,string[]>={producer:['artifact.create','task.create','task.submit_delivery','task.get'],reviewer:['task.get','review.get','artifact.read','review.start','review.submit'],observer:['task.get','review.get','workflow.history','artifact.read']}
  for(const tool of required[phase])assert.ok(calls.some(call=>call.tool===tool && call.status==='completed' && !call.errorCode && !call.transportError),`${phase}: missing successful ${tool}`)
  const refused:Record<Phase,string>={producer:'review.start',reviewer:'artifact.create',observer:'actor.create'}
  assert.ok(calls.some(call=>call.tool===refused[phase] && call.errorCode==='forbidden'),`${phase}: missing server-enforced permission denial for ${refused[phase]}`)
}

async function run() {
  let app=await createApp({directory:join(runDirectory,'data'),api:true,port:0})
  try {
    const operator=app.ctx.scope.bootstrap({projectName:'Codex live acceptance',actorName:'Test operator'})
    const caller:Caller={actorId:operator.actor.id,projectId:operator.project.id}
    const producer=app.ctx.scope.issueActor(caller,{name:'Fresh Codex producer',role:'producer'})
    const reviewer=app.ctx.scope.issueActor(caller,{name:'Fresh Codex reviewer',role:'reviewer'})
    const observer=app.ctx.scope.issueActor(caller,{name:'Fresh Codex observer',role:'reader'})
    writeFileSync(join(runDirectory,'credentials.json'),JSON.stringify({operator,producer,reviewer,observer},null,2)+'\n',{mode:0o600})
    await codex('producer',producer.token,app.ctx.api.url!,
    ...
    assert.ok(task,'Producer did not create a task');assert.equal(task.workflow.state,'in_review');assert.ok(task.reviewId)
    const firstTaskId=task.id,firstReviewId=task.reviewId
    ...
    console.log(JSON.stringify({phase:'restart-before-review',status:'verified',taskId:firstTaskId}))
    await codex('reviewer',reviewer.token,app.ctx.api.url!,
    ...
    assert.equal(task.workflow.state,'done');assert.equal(task.workflow.revision,2)
    const review=app.ctx.reviews.get(caller,firstReviewId)
    assert.equal(review.verdict,'pass');assert.equal(review.reviewerId,reviewer.actor.id);assert.notEqual(review.reviewerId,review.producerId)
    await app.stop()
    app=await createApp({directory:join(runDirectory,'data'),api:true,port:0})
    await codex('observer',observer.token,app.ctx.api.url!,
    ...
    assert.equal(new Set(summary.map(phase=>phase.threadId)).size,3,'Expected three distinct Codex instances')
    const evidenceChecks=verifyLiveEvidence(finalTask,app.ctx.reviews.get(caller,firstReviewId),{
    ...
    const report={status:'passed',directory:runDirectory,evidenceChecks,phases:summary,task:finalTask,review:app.ctx.reviews.get(caller,firstReviewId),history:app.ctx.workflows.history(caller,firstTaskId),events}
    await app.stop()
    writeFileSync(join(runDirectory,'report.json'),JSON.stringify(report,null,2)+'\n')
    console.log(JSON.stringify({status:'passed',report:join(runDirectory,'report.json'),phases:summary,taskId:firstTaskId}))
  } finally {await app.stop()}
}
run().catch(error=>{writeFileSync(join(runDirectory,'failure.json'),JSON.stringify({message:error.message,stack:error.stack,phases:summary},null,2)+'\n');console.error(error);process.exitCode=1})
