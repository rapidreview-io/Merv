...
// Reviews; it does not implement reflection consolidation or central publication.
const directory = resolve(process.argv[2] ?? `live-runs/code-${Date.now()}`);
...
const boot = await app.ctx.scope.bootstrap({
  projectName: 'Native code review acceptance',
  actorName: 'Owner',
});
const source = {
  ...
  return {
    ...
    build: async ({ caller, snapshot, tx }) => {
      ...
      const tools = readOnly
        ? ['review.get', 'artifact.read', 'step.review']
        : ['code.commit', 'code.operation', 'step.propose'];
      return {
    ...
    execution: {
      ...
      tools: readOnly
        ? [
            ...
            {
              name: 'artifact.read',
              alternatives: [{ artifactId: { kind: 'reference', name: 'proposalArtifactId' } }],
            },
            {
              name: 'step.review',
              alternatives: [
        ...
        : ['code.commit', 'code.operation', 'step.propose'].map((name) => ({
            name,
            alternatives: [{}],
          })),
      workspace: readOnly
...
const target = await program.start(source, { workflow: 'native-code-review', requestId: 'start' });
const handles = [
    ...
    handler: async (caller, input) =>
      await app.ctx.state.transaction(async (tx) => {
        ...
        const artifact = await app.ctx.artifacts.create(
          caller,
          {
            title: 'Immutable producer commit proposal',
            mediaType: 'application/json',
            content: JSON.stringify(
              { format: 1, command: operation.command, receipt: operation.receipt },
              null,
              2,
            ),
          },
        ...
        return await program.transition(
          caller,
          {
            ...
            data: {
              ...
              code: operation.receipt.headOid,
  ...
  app.ctx.tools.register({
    name: 'step.review',
    description:
      'Submit the actual independent assessment and apply it atomically. Passing completes this fixture; changes or failure reject it.',
    inputSchema: z
      .object({
        ...
        synopsis: z.string().min(40).max(420),
        findings: z
          .array(
            z
              .object({
                criterionNumber: z.number().int().min(1).max(2),
                status: z.enum(['met', 'not_met', 'not_verified']),
    ...
    handler: async (caller, input) =>
      await app.ctx.state.transaction(async (tx) => {
        ...
        check(
          snapshot.data.reviewId === input.reviewId && snapshot.state === 'review',
          'stale_review',
          'This is not the current review',
          409,
        );
        await app.ctx.reviews.submit(caller, { ...input, requestId: 'review-verdict' }, tx);
        return await program.transition(
          caller,
          {
            instanceId: target.id,
...
try {
  ...
  assert.equal(proposal.artifact.createdBy, sessions[0].actorId);
  assert.deepEqual(JSON.parse(proposal.content).receipt, operation.receipt);
  ...
  const report = {
    ...
    limits: [
      'Synthetic generic program with actual Reviews',
