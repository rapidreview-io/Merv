...
def find_worker(deadline, running):
...
def gate(prestart):
    # With prestart, the worker loads before any bootstrap exists and the launch hands it over.
    holder = loaded = None
    if prestart:
        holder = subprocess.Popen(
            ['/usr/bin/python3', '/opt/merv/runtime/start-runtime.py', '--prestart'],
            env={'PATH': '/usr/bin:/bin', 'LANG': 'C.UTF-8'}, stdin=subprocess.DEVNULL,
            stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, start_new_session=True,
        )
        loaded = find_worker(time.monotonic() + 20, lambda: holder.poll() is None)
        while not runtime.PRESTART.exists() and holder.poll() is None:
            time.sleep(0.05)
    ...
    try:
        ...
        worker = find_worker(deadline, lambda: parent.poll() is None)
        while (filename.exists() or runtime.PRESTART.exists()) and time.monotonic() < deadline:
            time.sleep(0.05)
        assert loaded in (None, worker), 'launch did not use the worker loaded at boot'
        ...
        assert not runtime.PRESTART.exists()
        ...
        supervisors = [parent.pid] + ([holder.pid] if holder else [])
        child = os.fork()
        if child == 0:
            ...
            for pid in supervisors:
                for target in [root_marker, Path(f'/proc/{pid}/environ')]:
                    try:
                        target.read_bytes()
                ...
                try:
                    ...
                    os._exit(3)
                if library.ptrace(16, pid, None, None) != -1 or ctypes.get_errno() not in (errno.EPERM, errno.EACCES):
                    os._exit(4)
        ...
        return {'gate': 'linux-pi-protected-launch', 'prestarted': prestart, 'uid': 12001,
                'capabilities': 0, 'noNewPrivileges': True, 'bootstrapRemoved': True,
        ...
        if holder:
            try:
                ...
                raise RuntimeError('prestart holder failed to stop')


for prestart in (False, True):
    print(json.dumps(gate(prestart)))
