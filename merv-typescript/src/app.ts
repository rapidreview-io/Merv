...
export const CORE_COMPONENTS = ['state','blobs','scope','artifacts','workflows','reviews','tasks','feed'] as const
export type Component = typeof CORE_COMPONENTS[number]
export interface AppOptions { directory: string; components?: Component[]; api?: boolean; host?: string; port?: number }
...
export async function createApp(options: AppOptions) {
  ...
  const add = async (plugin:any,config?:any) => {const fiber=await ctx.plugin(plugin,config);fibers.push(fiber);return fiber}
  const settle = async () => {
    ...
    const inactive=fibers.filter(fiber=>fiber.state !== FiberState.ACTIVE).map(fiber=>fiber.name)
    check(!inactive.length,'plugin_unavailable',`Plugins failed or have missing dependencies: ${inactive.join(', ')}`,503)
  }
  try {
    // Register consumers before providers to exercise Cordis dependency-driven activation.
    for (const [name,plugin,config] of [
      ['feed',feedPlugin],
      ['tasks',tasksPlugin], ['reviews',reviewsPlugin], ['artifacts',artifactsPlugin],
      ['workflows',workflowsPlugin], ['scope',scopePlugin],
      ['blobs',blobsPlugin,{root:join(directory,'blobs')}],
      ['state',statePlugin,{path:join(directory,'state.sqlite')}],
    ] as const) if (selected.has(name)) components.set(name,await add(plugin,config))
    await settle()
    if (options.api) {
      check(selected.has('scope'),'missing_dependency','API requires the scope component')
      await add(toolsPlugin)
      for (const [name,plugin] of [['scope',scopeToolsPlugin],['artifacts',artifactToolsPlugin],['workflows',workflowToolsPlugin],['reviews',reviewToolsPlugin],['tasks',taskToolsPlugin],['feed',feedToolsPlugin]] as const) if(selected.has(name)) adapters.set(name,await add(plugin))
    ...
    return {ctx,directory,components:components as ReadonlyMap<Component,Fiber>,adapters:adapters as ReadonlyMap<Component,Fiber>,stop:()=>stopping ??= (async()=>{
      const errors:unknown[]=[]
      try {if(options.api && ctx.api)await ctx.api.stop()} catch(error) {errors.push(error)}
