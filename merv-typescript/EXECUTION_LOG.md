...
- Step 6: in progress — Mounts implementation and controlled integration pass; authenticated real-service verification awaits explicit credential-use authorization.
...
## Step 6 — optional Mounts integration, live proof pending
...
- **Remaining gate:** authenticated real-service invocation by a fresh agent. Automatic approval review rejected the proposed use of the saved sandbox CLI token for `/v1/auth/me`; no credential was sent. A read-only harness is prepared for exact-origin identity verification and one selected `usage_report` call through Merv, with only aggregate verification evidence retained. It must be explicitly authorized and successfully executed before this step completes or step 7 begins.
- The user requested Claude Fable consultation after significant waves and allowed UI delegation to Fable. This preference is recorded in the plan. A pinned `claude-fable-5` review of the Mounts wave was prepared, but automatic approval review rejected sending the specific private source/test packet before the process launched. No Fable feedback exists yet; see the [review attempt and concrete packet](docs/reviews/step-06-fable.md). This disclosure authorization is separate from the pending sandbox credential authorization.

[Controlled mount-removal evidence](verification/step-06-controlled-mount-unload.json), [feed regression](verification/step-06-feed-unload.json), and [real sandbox preparation and command](docs/READ_ONLY_SANDBOX_MOUNT.md) separate fixture verification from the pending live-service proof.
