# Historical storage migration record

This records an earlier deployment and is not an executable runbook for the
current release. Its JWT credentials, namespace conventions and activation
commands have been retired. Use [the current cutover runbook](SANDBOXES_CUTOVER.md)
for account/grant migration. Retain this record only to interpret old storage
receipts and recovery evidence; use [ARTIFACT_R2_CUTOVER.md](ARTIFACT_R2_CUTOVER.md)
for Merv-owned research artifacts.
