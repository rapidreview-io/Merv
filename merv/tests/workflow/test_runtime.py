...
def test_branch_and_revision_loop_have_identical_transition_semantics(system):
...
def test_checked_branch_uses_durable_facts_for_guidance_and_enforcement(system):
    runtime, project_id = system
    ...
    with pytest.raises(WorkflowError, match="independent review must pass"):
...
def test_idempotent_transition_rejects_changed_commands_and_preserves_one_effect(system):
    runtime, project_id = system
    definition = Workflow(
    ...
    with pytest.raises(WorkflowError, match="different workflow action"):
...
def test_failed_history_write_rolls_back_state_and_requested_actions(system, monkeypatch):
    runtime, project_id = system
    runtime.registry.register(Workflow(
...
def test_definition_versions_are_pinned_until_explicit_migration(system):
    runtime, project_id = system
    ...
    with pytest.raises(ValueError, match="publish a new version"):
...
def test_node_context_uses_exact_references_and_dispatch_rechecks_revision(system):
    runtime, project_id = system
    ...
    act(runtime, project_id, current, "retry")
    with runtime.store.transaction() as conn, pytest.raises(WorkflowError, match="stale"):
...
def test_dispatch_prerequisites_do_not_invent_an_extra_workflow_state(system):
    runtime, project_id = system
    ...
    with pytest.raises(WorkflowError, match="dataset task"):
...
def test_parent_waits_for_every_child_and_routes_named_outcomes(system):
    runtime, project_id = system
...
def test_previous_wave_children_cannot_resume_the_new_wait(system):
    runtime, project_id = system
    runtime.registry.register(wave())
...
def test_start_retry_does_not_duplicate_children_or_follow_a_new_default_version(system):
    runtime, project_id = system
    runtime.registry.register(wave())
    ...
    with pytest.raises(WorkflowError, match="different workflow start"):
        runtime.start(project_id=project_id, workflow="wave", request_id="same-start", data={"different": True})


def test_delivery_retry_uses_stable_key_and_stale_worker_cannot_settle(system):
    runtime, project_id = system
    runtime.registry.register(Workflow(
...
def test_parent_version_migration_preserves_children_and_historical_membership(system):
    runtime, project_id = system
    runtime.registry.register(wave())
    ...
    with pytest.raises(WorkflowError, match="explicitly preserve"):
    ...
    with pytest.raises(WorkflowError, match="active children"):
        runtime.migrate(project_id=project_id, instance_id=parent.id, version=2,
    ...
    with pytest.raises(WorkflowError, match="declared child outcomes"):
...
def test_existing_child_composition_preserves_revision_and_rejects_second_parent(system):
    runtime, project_id = system
    ...
    with pytest.raises(WorkflowError, match="another workflow wait"):
...
def test_idempotency_conflict_is_checked_before_support_preparation(system):
    ...
    with pytest.raises(WorkflowError, match="request id"):
